<?php
   ...
   */
  class MakerSale extends BaseModel
  {
    /**
     * コンストラクタ
     */
    ...
    public function getOrdersList($where) {
      ...
      $sql .= " ORDER BY hed.order_time desc ";
   ...
   * @return   Boolean
   */
  public function updateOrderSales($pha_id,$ord_no) {
    global $DBA, $Log; // グローバル変数宣言
    $Log->trace("START updateOrderSales");

    $sql  = " UPDATE ord_t_head SET ";
    $sql .= "  kbn='2'";
    $sql .= " ,sale_time='".date("Y-m-d H:i:s")."'";
    ...
    if( $result === false )
    {
      $Log->trace("END updateOrderSales");
      return FALSE;
    }
    $Log->trace("END   updateOrderSales");
    return TRUE;
  ...
  * @return   Boolean
  */
 public function allOrderSales($arr = array()) {
   global $DBA, $Log; // グローバル変数宣言
   $Log->trace("START allOrderSales");
