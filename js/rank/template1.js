...
//メセージの所を作ります。
 ...
 //console.log(elem);
  ...
  t.innerHTML="";
  elem.appendChild(t); 
  elem_kbn_nm = document.getElementsByName("cust_rank_p_nm");
  for(i=0;i<elem_kbn_nm.length;i++){
      elem_kbn_nm[i].setAttribute("disabled", true);
  }
  
    elem_prate = document.getElementsByName("cust_rank_prate");
...
//行目を取得します。
    ...
    //主キーは空と地区名は変更した場合
    if(e.target.name === "cust_rank_cd" && e.target.value.replace(/\s/g,'') === ""){
        alert("※空白は禁止されています");
...
//主キーが空で一意ではないか確認します。
    ...
    if(pr_key_list.indexOf(e.target.name) !== -1){
        ...
        //同じ主キーあるを探します
            ...
            // 追加ボタンを管理します。            
            el_id = "cust_rank_cd_"+ref;
            cust_rank_cd_val = document.getElementById(el_id).value;
            if(cust_rank_cd_val !== "" ){
    ...
    else if(e.target.name === "cust_rank_cd" ){
        //地区名の場合
        if( e.target.value !== ""){
...
function T_f_keydown(e){
   ...
   if(e.key == "/" || e.key == "'" || e.key == '"' || e.key == "\\"){     
    ...
    if(e.target.name === "cust_rank_cd" || e.target.name === "cust_rank_prate" || e.target.name === "cust_rank_p_kbn"){
    ...
    else if(e.target.name === "cust_rank_prate"){
        elem=document.getElementById(e.target.id);
...
//            }
//        }        
    }else if(e.target.name === "cust_rank_p_kbn"){
        elem=document.getElementById(e.target.id);
        elem.value = elem.value.substr(0,1);        
        //console.log(e.key +"> 1 = " + (e.key > 1));
        if(e.key > 1){
            document.getElementById("message_box").innerHTML = " ※得点除外区分が不正です。【設定範囲:0～1】";
            return false;
        }else{
            document.getElementById("message_box").innerHTML = "";
            ref = e.target.id.split("kbn_")[1];
            elem_id = "cust_rank_p_nm_"+ref; 
            if(e.key == 0){
    ...
    }else if(e.target.name === "cust_rank_nm"){
        elem=document.getElementById(e.target.id);
...
function T_send_result(){
    ...
    var cd_id = "cust_rank_cd_"+elem_add.value;
    var nm_id = "cust_rank_nm_"+elem_add.value;
    //メセージを出します。
...
function f_check_formt_data(e){
    if(e.target.name == "cust_rank_cd" && e.target.value !== ""){
        document.getElementById(e.target.id).value=("0" + e.target.value).slice(-1);
    }
    if(e.target.name == "cust_rank_prate" && e.target.value !== ""){
       ...
       if(mes = 1){
           document.getElementById("message_box").innerHTML = "　※得点倍率の形式は99.9";
       }else{
           document.getElementById("message_box").innerHTML = "";
       }
    }
