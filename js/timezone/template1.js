...
function showdata(e){
    ...
    spath       = spath.split('?'); 
    spath       = spath[0]+'?param=TimeZone/show&org_id='+e.target.value;
...
if(document.getElementById("reset_data")){
	document.getElementById("reset_data").setAttribute("disabled", true);
}   
...
//所属店舗
elem_supp = document.getElementsByTagName("INPUT");


for(i=elem_supp.length-1;i>= 0;i--){ 
	//console.log(elem_supp[i].id);
	//console.log(i);
	supp_val    = elem_supp[i].value;
	//elem_supp[i].id = "to_delete_"+i;
	elem_supp[i].setAttribute("disabled", true);
	/*if(elem_supp[i].name === "tmzone_str" || elem_supp[i].name === "tmzone_end"){
		elem_supp[i].value = supp_val.replace(/(\d{2})(\d{2})(\d{2})/g, '$1:$2:$3');
