$(function() {
    $("#start_date, #end_date, #start_dateM, #end_dateM").bind('keydown', function (event) { 
        if(event.which == 13){ 
...
function creat_listener(){
    ...
    if(document.getElementById("txt_TRNYM")){document.getElementById("txt_TRNYM").addEventListener("change", f_date);}
    
    if(document.getElementById("start_date")){document.getElementById("start_date").addEventListener("blur", f_date);}
    if(document.getElementById("end_date")){document.getElementById("end_date").addEventListener("blur", f_date);}
    if(document.getElementById("saled_date")){document.getElementById("saled_date").addEventListener("blur", f_date);}
    if(document.getElementById("ord_date")){document.getElementById("ord_date").addEventListener("blur", f_date);}
    if(document.getElementById("saled_day")){document.getElementById("saled_day").addEventListener("blur", f_date);}
    if(document.getElementById("start_dateM")){document.getElementById("start_dateM").addEventListener("blur", f_date);}
    if(document.getElementById("end_dateM")){document.getElementById("end_dateM").addEventListener("blur", f_date);}
    if(document.getElementById("arr_date")){document.getElementById("arr_date").addEventListener("blur", f_date);}  
    if(document.getElementById("txt_TRNYM")){document.getElementById("txt_TRNYM").addEventListener("blur", f_date);}
    ...
    if(document.getElementById("serchForm")){
        document.getElementById("serchForm").onkeypress = function(e) {
...
//console.log(e);
    ...
    if("start_dateM" === e.target.id || "end_dateM" === e.target.id || "txt_TRNYM" === e.target.id){
        // 年月
        if(val.length === 4){
            if(isNaN(val)){
                //   YY/M
...
function f_table_sort(table_id,rownb){
    if(true){
        //normal sort
        sort_tbody(table_id,rownb);
    }else{
        // other sort
        data_send=[]
        send_asc=[];
...
function f_sort_tbody(table_id,rownb){
    ...
    if(document.getElementById(table_id)){
        elem_tbody=document.getElementById(table_id);
    }else{
        //other case
    }
    elem_tr = elem_tbody.getElementsByTagName('TR');
    if(elem_tr[elem_tr.length-1].innerText.indexOf('合計') !== -1 ){
        row_end = 2;
    }
    console.log(elem_tr);
    switching = true;
    ...
    while (switching) {
        ...
        for (i = 0; i < (rows.length - row_end); i++) {
            ...
            console.log('i= '+i);
            if(x){
                x = x.innerText.toLowerCase().replace(',','');
                console.log(x);
                x = ("0".repeat(50)+x).slice(-50);
            }
            if(y){
                y = y.innerText.toLowerCase().replace(',','');
                console.log(y);
                y = ("0".repeat(50)+y).slice(-50);
            }
            if(!(x && y)){
                continue;
            }  
