...
//行目を取得します。
    ...
    //主キーは空と地区名は変更した場合
    if(e.target.name === "cust_type_nm" && e.target.value.replace(/\s/g,'') === ""){
        alert("※空白は禁止されています");
...
//主キーが空で一意ではないか確認します。
    ...
    if(pr_key_list.indexOf(e.target.name) !== -1){
        ...
        //同じ主キーあるを探します
            ...
            // 追加ボタンを管理します。            
            el_id = "cust_type_nm_"+ref;
            cust_type_nm_val = document.getElementById(el_id).value;
            if(cust_type_nm_val !== "" ){
    ...
    else if(e.target.name === "cust_type_nm" ){
        //地区名の場合
        if( e.target.value !== ""){
...
function T_send_result(){
    ...
    var cd_id = "cust_type_cd_"+elem_add.value;
    var nm_id = "cust_type_nm_"+elem_add.value;
    //メセージを出します。
...
function f_check_formt_data(e){
    if(e.target.name == "cust_type_cd" && e.target.value !== ""){
