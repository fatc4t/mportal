...
function after_add(e){
    ...
    document.getElementById(elemt_id).setAttribute("disabled", true); 
/*
    $("#change_kbn_" + ref).css("imeMode", "disabled");
    $("#point_kbn_" + ref).css("imeMode", "disabled");
    $("#disc_money_" + ref).css("imeMode", "disabled");
    $("#gift_certi_cd_" + ref).css("imeMode", "disabled");
*/
}
//会社名
...
//メセージの所を作ります。
 ...
 //console.log(elem);
  ...
  for(i=0;i<elem_kbn_nm.length;i++){
   ...
   elem_cd = document.getElementsByName("gift_certi_cd");
   for(i=0;i<elem_cd.length;i++){ 
       $("#disc_money_" + i.toString()).css("imeMode", "disabled");
       $("#gift_certi_cd_" + i.toString()).css("imeMode", "disabled");
       $("#change_kbn_" + i.toString()).css("imeMode", "disabled");
       $("#point_kbn_" + i.toString()).css("imeMode", "disabled");
   }
...
//主キーが空で一意ではないか確認します。
    ...
    // 追加ボタンを管理します。            
            ...
            el_nm = "gift_certi_nm_"+ref;
            gift_certi_cd_val = document.getElementById(el_id).value;
            gift_certi_nm_val = document.getElementById(el_nm).value;
            if(gift_certi_cd_val !== "" && gift_certi_nm_val !== ""){
    ...
    if(A_pr_key_id.indexOf(e.target.name) !== -1){
        add_show    = "";   
        cur_val     = [];
        //主キー値を取得します。
        for(i=0;i<A_pr_key_id.length;i++){
        ...
        if (e.target.value.length === 1){
            document.getElementById(e.target.id).value = '0'+e.target.value;
        }        
        //地区名の場合
        if( e.target.value !== ""){
...
//console.log(e);
    ...
    if(e.target.name === "gift_certi_cd"){ 
        elem.value = e.target.value.substr(0,A_other['gift_size']);
        if(isNaN(elem.value)&&elem.value!==""){
           ...
           elem.focus();
            ...
            if(elem.value == 0 || elem.value > 98){
                document.getElementById("message_box").innerHTML = "　※ 商品券コードが不正です。【設定範囲:01～98】";
                elem.value = "";
                elem.focus();
            }else{
                document.getElementById("message_box").innerHTML = "";
        ...
        //console.log(elem.value.replace(',',''));
        elem.value = elem.value.replace(/[,\-\*.+]/g,'').substr(0,8);    
    }else if(e.target.name === "point_kbn"){
        //elem.value = elem.value.substr(0,1);        
        //console.log(elem.value +"> 1 = " + (elem.value > 1));
