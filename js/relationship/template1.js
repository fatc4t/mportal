...
//メセージの所を作ります。
 ...
 //console.log(elem);
  ...
  elem.appendChild(t); 
  if(working_data[0]){
      r_size = working_data[0]["relation_cd"].length;
  }else{
      r_size = 2;
  }
...
//行目を取得します。
    ...
    //主キーは空と地区名は変更した場合
    if(e.target.name === "relation_nm" && e.target.value.replace(/\s/g,'') === ""){
        alert("※空白は禁止されています");
...
//主キーが空で一意ではないか確認します。
    ...
    if(pr_key_list.indexOf(e.target.name) !== -1){
        ...
        //同じ主キーあるを探します
            ...
            // 追加ボタンを管理します。            
            el_id = "relation_nm_"+ref;
            relation_nm_val = document.getElementById(el_id).value;
            if(relation_nm_val !== "" ){
    ...
    else if(e.target.name === "relation_nm" ){
        //地区名の場合
        if( e.target.value !== ""){
...
function T_send_result(){
    ...
    var cd_id = "relation_cd_"+elem_add.value;
    var nm_id = "relation_nm_"+elem_add.value;
    //メセージを出します。
...
function f_check_formt_data(e){
    if(e.target.name == "relation_cd" && e.target.value !== ""){
        document.getElementById(e.target.id).value=("0".repeat(r_size) + e.target.value).slice(-r_size);
    }
}
