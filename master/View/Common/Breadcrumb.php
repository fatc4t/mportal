            ...
            <div class="menuNameArea">
                <h2>
                    ...
                    <?php 
                        ...
                        {
                            if( array_key_exists( "$pathShow", $_SESSION["M_PORTAL_SYS_MENU"] ) )
                            {
                                $breadcrumns["#"] = "管理者メンテナンス";
                                $breadcrumns[$pathShow] = $_SESSION["M_PORTAL_SYS_MENU"][$pathShow];
                            }
                        
                            if( array_key_exists( "$pathShow", $_SESSION["M_MANAGEMENT_MENU"] ) )
                            {
                                $breadcrumns["#"] = "管理者メンテナンス";
                                $breadcrumns[$pathShow] = $_SESSION["M_MANAGEMENT_MENU"][$pathShow];
                            }
                            
                            // 入力画面用のパスを作成
                            $inputPath = explode('&', $path);

                            if( array_key_exists( $inputPath[0], $_SESSION["M_INPUT_MENU"] ) )
                            {
                                $breadcrumns[$path] = $_SESSION["M_INPUT_MENU"][$inputPath[0]];
                            }
                        }
