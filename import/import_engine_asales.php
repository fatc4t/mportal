...
require_once("DBAccess_Function.php");
        ...
        // POSファイル保存パス
	$path = "../profit/import/acrossring/read/";
	...
	function read_file($comp_code, $filename, $filefull)
	{
		...
		$sql = "select * from ".$schema.".m_pos_key_file where is_del = 0 and pos_key_type = 0";
		...
		if ($fp){
			if (flock($fp, LOCK_SH)){
                                ...
                                // 同ファイル名にて、既に取込みが実施されていた場合は削除して上書き、それ以外は合算
                		$sql = "select * from ".$table_name." WHERE pos_code = '".$tenpo_code."' AND target_date = '".$yyyy.$mm.$dd."' AND file_name = '".$filename."' AND pos_key_file_id = '".$key_file_name."'";
                                $rows = getList($sql);

                                if($rows){
                                    // 取込み前に対象日の同じキーファイルタイプで該当組織分を上書きにするため削除
                                    $sql = "DELETE FROM ".$table_name." WHERE pos_code = '".$tenpo_code."' AND target_date = '".$yyyy.$mm.$dd."' AND file_name = '".$filename."' AND pos_key_file_id = '".$key_file_name."'";
                                    sqlExec($sql);
                                    ...
                                    // 取込み日のデータを上書きするので削除
                                    $sql = "DELETE FROM ".$table_mcode_name." WHERE target_date = '".$yyyy.$mm.$dd."' AND type = '1' AND pos_key_file_id = '".$key_file_name."' AND organization_id = '".$org_id."'";
                                    sqlExec($sql);
				...
				flock($fp, LOCK_UN);
                                ...
                                // mcode取込みを呼び出し
                                require("create_engine_asales.php");
