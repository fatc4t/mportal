...
require_once("DBAccess_Function.php");
        ...
        // POSファイル保存パス
	$path = "../profit/import/porushe/read/";
	...
	function read_file($comp_code, $filename, $filefull)
	{
		...
		for ($jj = 0; $jj < $key_file_cnt; $jj++){
			if (strstr($filename, $key_file[$jj])){
                                ...
                                //ファイル名から日付取り出し
				$tenpo_code = substr($filename, -17, 4);//店舗コード
				$yyyy = substr($filename, -12, 4);	//yyyy
				$mm = substr($filename, -8, 2);		//mm
				$dd = substr($filename, -6, 2);		//dd
		...
		if ($fp){
			if (flock($fp, LOCK_SH)){
				...
				flock($fp, LOCK_UN);
                                ...
                                // mcode取込みを呼び出し
                                require("create_engine_ptime.php");
