...
require_once("DBAccess_Function.php");
	...
	for ($jj = 0; $jj < $mapping_name_cnt; $jj++){
		//取得タイプのマッピングデータのみ
		$sql = "select * from ".$schema.". m_pos_mapping where mapping_name_id = '".$mapping_name[$jj][1]."' and is_del = 0 and logic_type = 0";
			...
			$mapping_data[$mapping_data_cnt][2] = "";			//キーファイル名
			...
			for ($jj = 0; $jj < $mapping_data_cnt; $jj++){
				...
				if ($flag == 1){
					$sql = "INSERT INTO ".$table_mcode_name." (";
						...
						$sql .= "'".$yyyy.$mm.$dd."', ";
						$sql .= "'".floatval($price)."', ";	//intval 浮動小数点なし
						$sql .= "'".$pos_key_file_id."', ";
