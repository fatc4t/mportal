...
require_once("DBAccess_Function.php");
        ...
        // POSファイル保存パス
	$path = "../profit/import/UOTERU/read/";
	...
	function read_file($comp_code, $filename, $filefull)
	{
		...
		$sql = "select * from ".$schema.".m_pos_key_file where is_del = 0 and pos_key_type = 1";
		...
		for ($jj = 0; $jj < $key_file_cnt; $jj++){
			if (strstr($filename, $key_file[$jj])){

                                //ファイル名から日付取り出し
				$tenpo_code = substr($filename, 0, 4);	//店舗コード
				$yyyy = substr($filename, 5, 4);	//yyyy
				$mm = substr($filename, 9, 2);		//mm
				$dd = substr($filename, 11, 2);		//dd
		...
		if ($fp){
			if (flock($fp, LOCK_SH)){
				...
				flock($fp, LOCK_UN);
                                ...
                                // mcode取込みを呼び出し
                                require("create_engine_time.php");
