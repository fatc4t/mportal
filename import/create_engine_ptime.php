				...
				if($t_key_file_name[$jj] == $row_import["pos_key_file_id"]){
						if (count($data) >= $col_limit[$jj] && $start_time[$jj] != ""){
							...
							if ($count[$jj] != ""){
                                                            ...
                                                            // order_detailから取得
                                                            // 取込みデータを取得
                                                            $sql_importOd = "select * FROM ".$schema.".t_import_data_day where target_date = '".$yyyy."/".$mm."/".$dd."'"
                                                                            ." and pos_key_file_id = 'order_detail' and pos_code = '".$tenpo_code."'";
                                                            $rows_importOd = getList($sql_importOd);

                                                            $row_cntOd = 0;
                                                            $count_set = 0;
                                                            while($row_importOd = $rows_importOd[$row_cntOd]) {
                                                                $dataOd = explode(",", $row_importOd["data_line"]);
                                                                // 同じ時間がどうかチェック
                                                                if ($data[$start_time[$jj]-1] == substr($dataOd[14], 11, 2)){
                                                                    $count_set += $dataOd[39];
                                                                }
                                                                $row_cntOd ++;
                                                            }
							...
							if ($sales[$jj] != ""){
                                                            ...
                                                            //orderから取得 下の売上(税込み)も同時に取得
                                                            // 取込みデータを取得
                                                            $sql_importO = "select * FROM ".$schema.".t_import_data_day where target_date = '".$yyyy."/".$mm."/".$dd."'"
                                                                            ." and pos_key_file_id = 'order_0' and pos_code = '".$tenpo_code."'";
                                                            $rows_importO = getList($sql_importO);

                                                            $row_cntO = 0;
                                                            $sales_tax_set = 0;
                                                            $sales_set = 0;
                                                            while($row_importO = $rows_importO[$row_cntO]) {
                                                                $dataO = explode(",", $row_importO["data_line"]);
                                                                // 同じ時間がどうかチェック
                                                                if ($data[$start_time[$jj]-1] == substr($dataO[29], 0, 2)){
                                                                    $sales_tax_set += $dataO[9];
                                                                    $sales_set += $dataO[9] - $dataO[8];
                                                                }
                                                                $row_cntO ++;
                                                            }
                                                        }
...
//                                                        }
								...
								$guest_count_set = $data[$guest_count[$jj]-1];
                                                        ...
                                                        if ($group_count[$jj] != ""){
                                                                // 通常伝票であれば組数1とする
								$group_count_set = $data[$group_count[$jj]-1];
                                                        }
