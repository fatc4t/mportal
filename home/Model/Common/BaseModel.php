<?php
    ...
    class BaseModel extends FwBaseModel
    {
         ...
         */
        public function getDisplayItemList( $postArray )
        {
            global $DBA, $Log; // グローバル変数宣言
            ...
            $sql = ' SELECT dd.output_type_id, dd.output_item_id, dd.output_item_branch, dd.item_name '
                 ...
                 . ' FROM m_display_item_detail dd WHERE dd.display_item_id = :display_item_id AND dd.is_display = 1 ORDER BY dd.disp_order ';
            ...
            while ( $data = $result->fetch(PDO::FETCH_ASSOC) )
            {
                array_push( $displayList, $data);
            }

            foreach( $displayList as $display )
            {
                if( $display['output_type_id'] == 1)
                {
                    if( !in_array( $display['output_item_id'], $postArray ) )
                    {
                        array_push( $displayItemList, $display);
                    }
                }
                else
                {
        ...
        protected function creatYearStartEndList( $startYear, $endYear )
        {
            ...
            {
                $periodDate = $this->searchAttendanceRecordOldestDate();
                list( $startYear, $startMonth, $startDay ) = explode("/", $periodDate);
            }
            if( empty( $endYear ) )
            {
                $periodDate = date("Y/m/d", strtotime("-1 day"));
                list( $endYear, $endMonth, $endDay ) = explode("/", $periodDate);
            }
         ...
         * @return   就業規則情報
         */
        protected function getLaborRegulationsAllInfo( $laborRegulationsID )
        {
            global $DBA, $Log; // グローバル変数宣言
            $Log->trace("START getLaborRegulationsAllInfo");
            
            // 就業規則IDから、就業規則適用期間IDを取得
            $appDateID = $this->getApplicationDateID( $laborRegulationsID );
        ...
        protected function getUserInfo( $userID )
        {
            ...
            // 従業員Noから、ユーザIDを取得
            $sql  = " SELECT user_id, employment_id, position_id, organization_id FROM v_user "
                  . " WHERE eff_code like '適用中' AND user_id = :user_id ";
         ...
         */
        private function getApplicationDateID( $laborRegulationsID )
        {
            global $DBA, $Log; // グローバル変数宣言
            ...
            $sql  = " SELECT application_date_id "
                  ...
                  . " WHERE labor_regulations_id = :labor_regulations_id AND eff_code = '適用中' ";
         ...
         * @return   残業時間マスタ情報
         */
        private function getOvertime ( $laborRegulationsID, $appDateID )
        {
            global $DBA, $Log; // グローバル変数宣言
            $Log->trace("START getHourlyWageChange");
