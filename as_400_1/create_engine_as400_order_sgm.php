...
/* * *****************************************************************************
  ...
  '*  修正履歴
  '*  修正日       修正者         修正内容
  '*  2021/11/12   バッタライ     原価売価0にならないよに設定
  '*  2021/11/12   バッタライ     部門コードが空白の場合0に変更
  '*  2021/11/12   バッタライ     全角入った場合は空白に変更する
  '*
  '***************************************************************************** */
...
try {
    ...
    // ログファイルパス
    $log_path = "/var/www/as_400_1/log/" . $logname . '.log';
    // 商品売価0の場合のログパス設定
    $log_path_baika = "/var/www/as_400_1/log/baika.log";
    // 空白
    $space = str_pad('', 5);
    // 処理開始ログ出力
...
//    $Dt_today  = date("D");
   ...
   //     $sql .= "   and TRN1601.CONNECT_KBN        = '0'                  "; // 未送信
        ...
        $sql .= "   and TRN1601.insdatetime  between '2021/11/16 00:00' and '2021/11/16 23:59'  ";
        $sql .= "order by                                                 ";
        $sql .= "    ORD_DATE                                             "; // 発注日
    ...
    //                update_m_denno($strBase_Firm_Cust_Cd, $strSeq);
                ...
                if($strCostprice <= 0){
                    // ログファイル出力し、つぎへ
                    error_log($currentdatetime . $space .
                              "商品コード　:".$space.db2str($row_import, 'prod_cd', 13), 3, $log_path_baika);
                    continue;
                }
                // 仕入単価(6)
...
error_log('END', 3, '/var/www/as_400_1/order/SGM.dat');
