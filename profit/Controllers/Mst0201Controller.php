<?php
     ...
     * @date      2018/11/15
     * @version   1.00
     * @note      POS商品マスタの新規登録/修正/削除を行う
     */

    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // POS商品マスタモデル
    require './Model/Mst0201.php';
    
    /**
     * POS商品マスタコントローラクラス
     * @note   ログイン時のリクエストを振り分ける
     */
    class Mst0201Controller extends BaseController
    {

        protected $mst0201 = null;     ///< POS商品マスタモデル
        ...
        public function showAction()
        {
            ...
            $Log->trace("START mst0201 showAction");

            $startFlag = true;

            $this->initialDisplay($startFlag);
            $Log->trace("END mst0201 showAction");
        }
        ...
        {
            global $Log; // グローバル変数宣言
            $Log->trace( "START mst0201 searchAction" );
            ...
            $Log->trace("END mst0201 searchAction");
        ...
        {
            global $Log; // グローバル変数宣言
            $Log->trace( "START mst0201 addInputAction" );

            $this->Mst0201InputPanelDisplay();

            $Log->trace("END mst0201 addInputAction");
        ...
        {
            global $Log; // グローバル変数宣言
            $Log->trace( "START mst0201 addAction" );
            ...
            // 新規登録
            $message = $mst0201->addNewData($postArray);
            ...
            $Log->trace("END mst0201 addAction");
        }
        ...
        {
            global $Log; // グローバル変数宣言
            $Log->trace( "START mst0201 modAction" );
            ...
            // 更新
            $message = $mst0201->updateData($postArray);
            ...
            $Log->trace("END mst0201 modAction");
        }
        ...
        {
            global $Log; // グローバル変数宣言
            $Log->trace( "START mst0201 delAction" );
            ...
            // 記事を削除
            $message = $mst0201->delData($postArray);
            ...
            $Log->trace("END mst0201 delAction");
        }
        ...
        {
            global $Log;  // グローバル変数宣言
            $Log->trace("START mst0201 initialDisplay");

            // グループマスタモデルインスタンス化
            $mst0201 = new Mst0201();

            // 検索用プルダウン
            $abbreviatedNameList    = $mst0201->setPulldown->getSearchOrganizationList( 'reference', true, true );      // 組織略称リスト
            
            // 検索用商品プルダウン
            $mst0201PulldownList = $mst0201->setPulldown->getSearchMst0201List();
            // 検索用商品プルダウン（JSON）
            $json_mst0201PulldownList = json_encode($mst0201PulldownList, JSON_UNESCAPED_UNICODE);            
            ...
            // POS商品マスタ一覧データ取得
            $mst0201AllList = $mst0201->getListData($searchArray);
            // POS商品マスタレコード数
            $mst0201RecordCnt = count($mst0201AllList);
            // 表示レコード数
            ...
            if($startFlag){
                // 初期表示の場合
                $pageNo = $this->getDuringTransitionPageNo($mst0201RecordCnt, $pagedRecordCnt);
            }else{
                // その他の場合
            ...
            $idName = "prod_cd";
            // 一覧表
            $mst0201List = $this->refineListDisplayNoSpecifiedPage($mst0201AllList, $idName, $pagedRecordCnt, $pageNo);
            $mst0201List = $this->modBtnDelBtnDisabledCheck($mst0201List);

            // 表示レコード数が11以上の場合、スクロールバーを表示
            $isScrollBar = false;
            if( $_SESSION["DISPLAY_RECORD_CNT"] >= 15 && count($mst0201List) >= 15)
            {
                $isScrollBar = true;
            }
            ...
            // ページ数
            $pagedCnt = ceil($mst0201RecordCnt /  $pagedRecordCnt);
            ...
            if($startFlag){
                ...
                // NO表示用
                $display_no = 0;
                // ソートがを選択されたかどうかのチェックフラグ（Mst0201TablePanel.htmlにて使用）
                $mst0201NoSortFlag = false;
            }else{
                // 対象項目へマークを付与
                $headerArray = $this->changeHeaderItemMark(parent::escStr( $_POST['sortConditions'] ));
                // NO表示用
                $display_no = $this->getDisplayNo( $mst0201RecordCnt, $pagedRecordCnt, $pageNo, parent::escStr( $_POST['sortConditions'] ) );
                // ソートがを選択されたかどうかのチェックフラグ（Mst0201TablePanel.htmlにて使用）
                $mst0201NoSortFlag = $searchArray['sort'] % 2 == 1 ? true : false;
            }

            if($startFlag){
                require_once './View/Mst0201Panel.html';
            }else{
                require_once './View/Mst0201TablePanel.html';
            }

            $Log->trace("END mst0201 initialDisplay");
        }
         ...
         * @return   無
         */
        private function mst0201InputPanelDisplay()
        {
            global $Log, $TokenID; // グローバル変数宣言
            $Log->trace( "START mst0201InputPanelDisplay" );

            $mst0201 = new Mst0201();

            $mst0201DataList = array();
            // 配列初期化
            $mst0201->initMst0201DataList($mst0201DataList);

            // 修正時フラグ（false）
            $CorrectionFlag = false;
            // 登録データ更新時
            if(!empty($_POST['mst0201pk']))
            {
                $mst0201pk = $_POST['mst0201pk'];
                $mst0201Param = explode( ':', $mst0201pk );
                $organization_id    = $mst0201Param[0];
                $prod_cd            = $mst0201Param[1];

                // POS商品マスタデータを取得
                $mst0201DataList = $mst0201->getMst0201Data( $organization_id, $prod_cd );
...
//            $mst1201taxinfo['sect_tax_type']    = $mst0201DataList['sect_tax_type'];
//            $mst1201taxinfo['sect_tax']         = $mst0201DataList['sect_tax'];
//            // (部門)税区分ラベル
...
//            }
            ...
            // 原価
            $costprice = floatval(str_replace(',', '', $mst0201DataList['head_costprice']));
            // 売価(税抜)
            $saleprice = floatval(str_replace(',', '', $mst0201DataList['saleprice_ex']));
            // 粗利額：売価 - 原価            
            ...
            // 粗利額
            $mst0201DataList['profit'] = number_format($profit);
            // 粗利率
            $mst0201DataList['profit_per'] = number_format($profit_per, 2);
            
            // 簡易特売期間をスラッシュ分割
            if (isset($mst0201DataList['smp1_str_dt']) === true && $mst0201DataList['smp1_str_dt'] !== '') {
                $mst0201DataList['smp1_str_dt'] = substr($mst0201DataList['smp1_str_dt'],0,4).'/'.substr($mst0201DataList['smp1_str_dt'],4,2).'/'.substr($mst0201DataList['smp1_str_dt'],6,2);
            }
            if (isset($mst0201DataList['smp1_end_dt']) === true && $mst0201DataList['smp1_end_dt'] !== '') {
                $mst0201DataList['smp1_end_dt'] = substr($mst0201DataList['smp1_end_dt'],0,4).'/'.substr($mst0201DataList['smp1_end_dt'],4,2).'/'.substr($mst0201DataList['smp1_end_dt'],6,2);
            }
            if (isset($mst0201DataList['smp2_str_dt']) === true && $mst0201DataList['smp2_str_dt'] !== '') {
                $mst0201DataList['smp2_str_dt'] = substr($mst0201DataList['smp2_str_dt'],0,4).'/'.substr($mst0201DataList['smp2_str_dt'],4,2).'/'.substr($mst0201DataList['smp2_str_dt'],6,2);
            }
            if (isset($mst0201DataList['smp2_end_dt']) === true && $mst0201DataList['smp2_end_dt'] !== '') {
                $mst0201DataList['smp2_end_dt'] = substr($mst0201DataList['smp2_end_dt'],0,4).'/'.substr($mst0201DataList['smp2_end_dt'],4,2).'/'.substr($mst0201DataList['smp2_end_dt'],6,2);
            }
            
            // ブロック1を分割
            if (isset($mst0201DataList['shelf_block1']) === false || $mst0201DataList['shelf_block1'] === '') {
                ...
                $strTmp = str_pad($mst0201DataList['shelf_block1'], 9, '0', STR_PAD_RIGHT);
                $array_shelf_block = array(
            ...
            $mst0201DataList = array_merge($mst0201DataList, $array_shelf_block);
            
            // ブロック2を分割
            if (isset($mst0201DataList['shelf_block2']) === false || $mst0201DataList['shelf_block2'] === '') {
                $array_shelf_block = array(
                        'shelf_block2_1' => '',
                        'shelf_block2_2' => '',
                        'shelf_block2_3' => ''
                ...
                $strTmp = str_pad($mst0201DataList['shelf_block2'], 9, '0', STR_PAD_RIGHT);
                $array_shelf_block = array(
            ...
            // 棚番1を分割
            if (isset($mst0201DataList['shelf_no'][0]) === false || $mst0201DataList['shelf_no'][0] === '') {
                ...
                $strTmp = str_pad($mst0201DataList['shelf_no'][0], 9, '0', STR_PAD_RIGHT);
                $array_shelf_no = array(
            ...
            $mst0201DataList = array_merge($mst0201DataList, $array_shelf_no);

            // 棚番2を分割
            if (isset($mst0201DataList['shelf_no'][1]) === false || $mst0201DataList['shelf_no'][1] === '') {
                $array_shelf_no = array(
                        'shelf_no2_1' => '',
                        'shelf_no2_2' => '',
                        'shelf_no2_3' => ''
                ...
                $strTmp = str_pad($mst0201DataList['shelf_no'][1], 9, '0', STR_PAD_RIGHT);
                $array_shelf_no = array(
            ...
            // 棚番3を分割
            if (isset($mst0201DataList['shelf_no'][2]) === false || $mst0201DataList['shelf_no'][2] === '') {
                $array_shelf_no = array(
                        'shelf_no3_1' => '',
                        'shelf_no3_2' => '',
                        'shelf_no3_3' => ''
                ...
                $strTmp = str_pad($mst0201DataList['shelf_no'][2], 9, '0', STR_PAD_RIGHT);
                $array_shelf_no = array(
            ...
            // 登録用所属組織プルダウン
            $abbreviatedList        = $mst0201->setPulldown->getSearchOrganizationList( 'registration', true, true );
            // 登録用POS商品部門プルダウン
            $mst1201DataList        = $mst0201->setPulldown->getSearchMst1201List( $organization_id );
            // 登録用POS商品仕入先プルダウン
            $mst1101DataList        = $mst0201->setPulldown->getSearchMst1101List( $organization_id );
            // 登録用POS商品自社分類プルダウン
            $mst5501DataList        = $mst0201->setPulldown->getSearchMst5501List( $organization_id );
            // 登録用POS商品JICFS分類プルダウン
            $mst5401DataList        = $mst0201->setPulldown->getSearchMst5401List( $organization_id );
            // 登録用POS商品メーカープルダウン
            $mst1001DataList        = $mst0201->setPulldown->getSearchMst1001List( $organization_id );
            // 登録用POS商品区分プルダウン
            $mst0901DataList        = $mst0201->setPulldown->getSearchMst0901List( $organization_id );
            // POS商品分類プルダウン
            $mst0801DataList        = $mst0201->setPulldown->getSearchMst0801List( $organization_id );
            ...
            // POSシステムマスタ取得
            $mst0010DataList = $mst0201->getMst0010Data( $organization_id );
            // POSシステムマスタ：店舗コード
            $mst0201DataList['shop_cd'] = $mst0010DataList['shop_cd'];
            // POSシステムマスタ：消費税端数処理区分(0:切り捨て/1:四捨五入/2:切り上げ)
            $mst0201DataList['tax_frac_kbn'] = $mst0010DataList['tax_frac_kbn'];
            

            require_once './View/Mst0201InputPanel.html';
            
            $Log->trace("END mst0201InputPanelDisplay");
        }
        ...
        {
            global $Log;  // グローバル変数宣言
            $Log->trace("START mst0201 changeHeaderItemMark");
            ...
            if(!empty($sortNo))
            {
                $sortList[1]  = "mst0201NoSortMark";
                $sortList[2]  = "mst0201NoSortMark";
                ...
                $sortList[13] = "sectNameSortMark";
                $sortList[14] = "sectNameSortMark";
                $sortList[15] = "suppNameSortMark";
                $sortList[16] = "suppNameSortMark";
