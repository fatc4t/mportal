<?php
    ...
    // Login処理モデル
    require './Model/LedgerSheetDay.php';
     ...
     */
    class LedgerSheetDayController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        public function csvoutputAction()
        {
            ...
            $ledgerSheetDay = new ledgerSheetDay();
            $abbreviatedNameList = $ledgerSheetDay->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
            ...
            // 帳票フォーム一覧データ取得
            $ledgerSheetDayList = $ledgerSheetDay->getFormListData($searchArray);
...
//            array_push($dispDateList, $dPeriod);
            ...
            foreach($dateList as $dateListOne){
                ...
                for ($hc = 0; $hc < count($logic); $hc++){
                    ...
                    if($logic[$hc] == 1){
                        // 前年だったら日付を操作
                        $searchDay = date($dateListOne["days"],strtotime("-1 year"));
                    }else{
                        // 当年だったらそのまま設定
                    ...
                    // Ｍコード一覧データ取得
                    $mCodeDataList = $ledgerSheetDay->getMcodeData($searchMArray);
                    ...
                    if($mCodeLogic[0] == '#'){
                        ...
                        // Ｍコード一覧データ取得
                        $mCodeDataDayList = $ledgerSheetDay->getReportDataDay($searchrdArray);
                        ...
                        for ($cs = 0; $cs < count($calStr); $cs++){
                            ...
                            // Mコードかどうかチェック
                            $checkMcode = $ledgerSheetDay->checkMcode($searchrdArray);
                            ...
                            if(!empty($checkMcode)){
                                if($calStr[$cs] == "172001" || $calStr[$cs] == "172002" || $calStr[$cs] == "172999"){
                                    ...
                                    if($logic[$hc] == 1){
                                        ...
                                        $eDate = date('Y/m/d', strtotime('+1 day', $sDate));
                                    }else{
                                        // 当年だったらそのまま設定
                                    ...
                                    $mEntity = $mEntity + array('val' => $entity);
                                        ...
                                        // Ｍコード一覧データ取得
                                        $mCodeDataListSub = $ledgerSheetDay->getMcodeData($searchMArraySub);
                                        ...
                                        if($mCodeLogicSub[0] == '#'){
                                            ...
                                            // Ｍコード一覧データ取得
                                            $mCodeDataDayListSub = $ledgerSheetDay->getReportDataDay($searchMArraySub);
                        ...
                        // Ｍコード一覧データ取得
                        $mCodeDataDayList = $ledgerSheetDay->getMcodeListData($searchMlArray);
...
//            array_push($mcodeList, $sumList);
            ...
            // 先頭固定ヘッダ追加
            array_push($export_csv_title, "対象日");
            ...
            header("Content-Disposition: attachment; filename=売上分析帳票".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
...
//            array_push($mcodeList, $sumList);
            ...
            if($mode == "show" || $mode == "initial"){
                require_once './View/LedgerSheetDayPanel.html';
            }else{
                require_once './View/LedgerSheetDayEXPanel.html';
            }
