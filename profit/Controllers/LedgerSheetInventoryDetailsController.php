<?php
    ...
    // Login処理モデル
    require './Model/LedgerSheetInventoryDetails.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetInventoryDetailsController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        private function initialDisplay($mode)
        {
            ...
            $LedgerSheetInventoryDetails = new LedgerSheetInventoryDetails();
            //モーダル
            $modal = new Modal();
            //店舗指定モーダル取得 
            $org_id_list    = [];
            ...
            require_once './View/LedgerSheetInventoryDetails.html';
        ...
        public function getdataAction($mode)
        {
            ...
            $LedgerSheetInventoryDetails = new LedgerSheetInventoryDetails();
            $data['dataget'] = $LedgerSheetInventoryDetails->get_data($joken);
 ...
 /**
        ...
        public function csvoutputAction()
        {
            ...
            header("Content-Disposition: attachment; filename=入庫明細表".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
