...
//*****************************************************************************
    ...
    // Login処理モデル
    require './Model/LedgerSheetShopDay.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetShopDayController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        public function pdfoutputAction()
        {
            ...
            $ledgerSheetShopDay = new ledgerSheetShopDay();
            ...
            //テスト用
            //$html = file_get_contents("./View/LedgerSheetShopDayPDFPanel.html");
            ...
            $list = $ledgerSheetShopDay->getFormListData($searchArray);
        ...
        private function initialDisplay($mode)
        {
            ...
            //$abbreviatedNameList = $ledgerSheetShopDay->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
            // modal
            $modal = new Modal();
            $org_id_list    = [];
           ...
           // $endDate = date('Y/m/d', strtotime('last day of ' . $startDate));
            ...
            if($mode != "initial") {
                // 帳票フォーム一覧データ取得
                $ledgerSheetDetailList = $ledgerSheetShopDay->getFormListData($searchArray);
                ...
                if(count($ledgerSheetDetailList) > 0){
                    ...
                    foreach($ledgerSheetDetailList as $data){
                        ...
                        $sumTotalTax_08   +=  $data["tax_total_08"];
                        $sumTotalTax_10   +=  $data["tax_total_10"];
                        $sumTotalHiki   = $sumTotalHiki + $data["hiki_total"];
                        $sumsaletotal   += $data["sale_total"]; 
                    }

                    $sumLine = array(
                        'proc_date'          => '総合計',            // 合計名称
                        'pure_total_i'       => $sumPureTotalI,    // 売価金額
                        'composition_ratio_pure_total_i'  => '',   // 構成比
                        'total_amount'       => $sumTotalAmount,   // 数量
                        'composition_ratio_total_amount'  => '',    // 構成比
                        ...
                        'tax_total_08'          => $sumTotalTax_08,                 // 消費税8%
                        'tax_total_10'          => $sumTotalTax_10,                 // 消費税
                        'hiki_total'         => $sumTotalHiki,                 // 引額
                        'sale_total'         => $sumsaletotal
                        );
            ...
            {
                require_once './View/LedgerSheetShopDayPanel.html';
            }
        }
        ...
        public function csvoutputAction()
        {
            ...
            array_push($export_csv_title, "消費税10%"); // @m1 2019/03/15 モンターニュ add
