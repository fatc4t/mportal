<?php
     ...
     * @date      2016/06/30
     * @version   1.00
     * @note      目標設定月次の新規登録/修正/削除を行う
     */

    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // Login処理モデル
    require './Model/TergetReportListMonth.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class TergetReportListMonthController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        public function addAction()
        {
            ...
            $tergetReportListMonth = new TergetReportListMonth();
            ...
            // 報告書フォーム一覧データ取得
            $salesFormDetailList = $tergetReportListMonth->getFormDetailList($searchArray);
            ...
            foreach($salesFormDetailList as $data){
                ...
                $messege = $tergetReportListMonth->addNewData($inputArray);
            }
        ...
        public function modAction()
        {
            ...
            foreach($salesFormDetailList as $data){
                ...
                $messege = $tergetReportListMonth->modUpdateData($updateArray);
            }
        ...
        public function delAction()
        {
            ...
            $messege = $tergetReportListMonth->del($deleteArray);
        ...
        private function initialDisplay()
        {
            ...
            $tergetReportListMonth = new TergetReportListMonth();
            $abbreviatedNameList = $tergetReportListMonth->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
            ...
            // 帳票フォーム一覧データ取得
            $tergetReportMonthList = $tergetReportListMonth->getFormListData($searchArray);

            // ヘッダ情報、Ｍコード情報取り出し 
            foreach($tergetReportMonthList as $data){
            ...
            $searchMArray = array(
                ...
                'report_form_id'    => '5',                             // 報告書設定ID
            );

            // 詳細情報と登録済みデータを取得
            $tergetReportMonthDetailList = $tergetReportListMonth->getFormDetailListData($searchMArray);

            // ヘッダ情報、Ｍコード情報取り出し 
            foreach($tergetReportMonthDetailList as $dData){
                if($dData["data"] != ""){
                    $dataFlg = "true";
                    break;
                }
            ...
            require_once './View/TergetReportListMonthPanel.html';
        }
