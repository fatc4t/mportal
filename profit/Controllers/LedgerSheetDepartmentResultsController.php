<?php
    ...
    // コスト帳票処理モデル
    require './Model/LedgerSheetDepartmentResults.php';
     ...
     */
    class LedgerSheetDepartmentResultsController extends BaseController
    {
        /**
         * コンストラクタ
         */
 ...
 /**
        ...
        public function csvoutputAction()
        {
            ...
            header("Content-Disposition: attachment; filename=部門別実績".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
        ...
        private function initialDisplay($mode)
        {
            ...
            $ledgerSheetDepartmentResults = new LedgerSheetDepartmentResults();
            $modal = new Modal();

            // 帳票フォーム一覧データ取得
            $data           = [];
            $data           = $ledgerSheetDepartmentResults->get_data();
            $org_id_list       = [];
            $org_id_list    = $modal->getorg_detail();
            $dept_detail    = [];
            $dept_detail    = $modal->getsect_detail();
            $Sizelist       = [];
            $base           = new BaseModel();
            ...
            require_once './View/LedgerSheetDepartmentResultsPanel.html';
