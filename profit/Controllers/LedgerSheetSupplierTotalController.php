<?php
    ...
    // Login処理モデル
    require './Model/LedgerSheetSupplierTotal.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetSupplierTotalController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        private function initialDisplay($mode)
        {
            ...
            $ledgerSheetSupplierTotal = new ledgerSheetSupplierTotal();

            $abbreviatedNameList = $ledgerSheetSupplierTotal->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
            ...
            $supplier = parent::escStr( $_POST['supp_nm'] );
            $searchArray = array(
            ...
            if($mode != "initial") {
                $ledgerSheetSupplierTotalList = $ledgerSheetSupplierTotal->getFormListData($searchArray);
            }
            $Sizelist = [];
            $base = new BaseModel();
            ...
            {
                require_once './View/LedgerSheetSupplierTotalPanel.html';
            }
        }
        ...
        public function csvoutputAction()
        {
            ...
            $ledgerSheetSupplierTotal = $ledgerSheetSupplierTotal->getFormListData($searchArray);
            ...
            if( touch($file_path) ){
                ...
                foreach( $ledgerSheetSupplierTotal as $rows ){
		    ...
		    $arari = round($rows['saleprice'] - $rows['costtotal'],0);
                    $str = $str.'","'.$arari;
                    $str = $str.'","'.round($arari / $rows['saleprice']*100,2);
                    $str = $str.'"';
            ...
            header("Content-Disposition: attachment; filename=仕入実績".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
