<?php
    ...
    // Login処理モデル
    require './Model/LedgerSheetProductDelayedUpdate.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetProductDelayedUpdateController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        private function initialDisplay($mode)
        {
            ...
            $ledgerSheetProductDelayedUpdate = new ledgerSheetProductDelayedUpdate();

            $abbreviatedNameList = $ledgerSheetProductDelayedUpdate->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
            ...
            if($mode != "initial")
            {
                $ledgerSheetProductDelayedUpdateList = $ledgerSheetProductDelayedUpdate->getFormListData($searchArray);
            }
            $Sizelist = [];
            $base = new BaseModel();
            ...
            {
                require_once './View/LedgerSheetProductDelayedUpdatePanel.html';
            }
        }
        ...
        public function csvoutputAction()
        {
            ...
            array_push($export_csv_title, "予約日付");
            array_push($export_csv_title, "新値有効");
            ...
            if( touch($file_path) ){
                ...
                foreach( $ledgerSheetProductDelayedUpdateList as $rows ){
		    ...
		    if ($rows['noreturn_kbn'] == '0'){
                    ...
                    // 内容行を1行ごとにCSVデータへ
                    $file = fopen($file_path, 'a'); fwrite($file,$str."\n");
		    }
           }
            ...
            header("Content-Disposition: attachment; filename=予約商品登録一覧".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
