<?php
    ...
    // Login処理モデル
    require './Model/LedgerSheetProductHistory.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetProductHistoryController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        public function pdfoutputAction()
        {
            ...
            $ledgerSheetProductHistory = new ledgerSheetProductHistory();
            ...
            $list = $ledgerSheetProductHistory->getFormListData($searchArray);
        ...
        private function initialDisplay($mode)
        {
            ...
            $abbreviatedNameList = $ledgerSheetProductHistory->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
...
*/
            ...
            if($mode != "initial") {
                // 帳票フォーム一覧データ取得
                //$ledgerSheetDetailList = $ledgerSheetProductHistory->getFormListData($searchArray);
                $data = $ledgerSheetProductHistory->getFormListData($searchArray);
            }
            $searchArray = $_POST;
            $searchArray['org_id'] = str_replace("'","",$_POST['org_id']);
            $searchArray['prod_cd'] =  str_replace("'","",$_POST['prod_cd']);
            $searchArray['sect_cd'] =  str_replace("'","",$_POST['sect_cd']);
            $searchArray['start_date'] =  $startDate;
            $searchArray['end_date'] =  $endDate;
            $searchArray['csv_data'] = '';

            // 初期表示のときは、全てのチェックをONにする
            if($mode === "initial"){
                $searchArray['history_chk_hai'] = 'true';
                $searchArray['history_chk_haie'] = 'true';
                $searchArray['history_chk_ji'] = 'true';
                $searchArray['history_chk_jie'] = 'true';
                $searchArray['history_chk_mi'] = 'true';
                $searchArray['history_chk_mie'] = 'true';
                $searchArray['history_chk_nyu'] = 'true';
                $searchArray['history_chk_sa'] = 'true';
                $searchArray['history_chk_sae'] = 'true';
                $searchArray['history_chk_shi'] = 'true';
                $searchArray['history_chk_shie'] = 'true';
                $searchArray['history_chk_shu'] = 'true';
                $searchArray['history_chk_ta'] = 'true';
                $searchArray['history_chk_u'] = 'true';
                $searchArray['history_chk_ue'] = 'true';                  
            }
            ...
            {
                require_once './View/LedgerSheetProductHistoryPanel.html';
            }
        }
        ...
        public function csvoutputAction()
        {
            ...
            header("Content-Disposition: attachment; filename=商品履歴表".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
