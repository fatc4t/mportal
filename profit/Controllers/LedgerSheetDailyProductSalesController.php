<?php
    ...
    // コスト帳票処理モデル
    require './Model/LedgerSheetDailyProductSales.php';
     ...
     */
    class LedgerSheetDailyProductSalesController extends BaseController
    {
        /**
         * コンストラクタ
         */
 ...
 /**
        ...
        public function csvoutputAction()
        {
            ...
            header("Content-Disposition: attachment; filename=特売商品売上日報".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
        ...
        private function initialDisplay($mode)
        {
            ...
            $ledgerSheetDailyProductSales = new LedgerSheetDailyProductSales();
            $modal = new Modal();

            // 帳票フォーム一覧データ取得
            $data           = [];
            $data           = $ledgerSheetDailyProductSales->get_data();
            $org_id_list    = [];
            $org_id_list    = $modal->getorg_detail();
            $sect_detail    = [];
            $sect_detail    = $modal->getsect_detail();  
            $splan_detail    = [];
            $splan_detail    = $modal->getsplan_detail();             
            $Sizelist       = [];
            $base           = new BaseModel();
            ...
            require_once './View/LedgerSheetDailyProductSalesPanel.html';
