<?php
    /**
     * @file      帳票 - 仕入単品分析コントローラ
     * @author    millionet oota
     * @date      2018/06/04
     * @version   1.00
     * @note      帳票 - 仕入単品分析の閲覧を行う
     */

    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // Login処理モデル
    require './Model/LedgerSheetPurchaseDay.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetPurchaseDayController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        public function pdfoutputAction()
        {
            ...
            $ledgerSheetPurchaseDay = new ledgerSheetPurchaseDay();
            ...
            //テスト用
            //$html = file_get_contents("./View/LedgerSheetPurchaseDayPDFPanel.html");
        ...
        private function initialDisplay($mode)
        {
            ...
            //$abbreviatedNameList = $ledgerSheetPurchaseDay->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
            // modal
            $modal = new Modal();
            $org_id_list    = [];
            ...
            if($mode != "initial") {
                // 帳票フォーム一覧データ取得
                $ledgerSheetDetailList = $ledgerSheetPurchaseDay->getFormListData($searchArray);
                ...
                if(count($ledgerSheetDetailList) > 0){
                    ...
                    foreach($ledgerSheetDetailList as $data){
                        ...
                        $sumSaleprice = $sumSaleprice + $data["saleprice"];
                        $sumArari = $sumArari + $data['gross_profit'];
                        $sumSale = $sumSale + $data['saletotal'];
                    }
                    if (empty($sumAmount)) {
                        $sumAmount = null;
                    }
                    ...
                    $sumLine = array(
                        ...
                        'stock_date'            => '合計',            // 仕入日付
                        'amount'                => $sumAmount,    // 数量
                        'costprice'             => '', //$sumCostprice,   // 単価
                        'costtotal'             => $sumCosttotal,  // 単価合計
                        'saleprice'             => '', // $sumSaleprice,  // 売上金額
                        'saletotal'             => $sumSale,  // 売上合計
                        'gross_profit'          => $sumArari,   // 粗利
                        'gross_profit_margin'   => number_format($sumArari / $sumSale * 100,2) // 粗利率
                        ); 
            ...
            {
                require_once './View/LedgerSheetPurchaseDayPanel.html';
            }
        }
        ...
        public function csvoutputAction()
        {
            ...
            array_push($export_csv_title, "仕入日");
            ...
            if( touch($file_path) ){
                ...
                foreach( $ledgerSheetDetailList as $rows ){
                    ...
                    $str = $str.'","'.round($rows['saletotal'],0);
                    $str = $str.'","'.round($rows['gross_profit'],0);
                    $str = $str.'","'.round($rows['gross_profit_margin'],2);
                    $str = $str.'"';
            ...
            header("Content-Disposition: attachment; filename=仕入単品分析".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
