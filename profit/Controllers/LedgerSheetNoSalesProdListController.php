<?php
     ...
     * @date      2019/06/04
     * @version   1.00
     * @note      帳票 - ゼロ商品一覧表の閲覧を行う
     */

    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // Login処理モデル
    require './Model/LedgerSheetNoSalesProdList.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetNoSalesProdListController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        public function pdfoutputAction()
        {
            ...
            $LedgerSheetNoSalesProdList = new LedgerSheetNoSalesProdList();
            ...
            //テスト用
            //$html = file_get_contents("./View/LedgerSheetNoSalesProdListPDFPanel.html");
            ...
            $list = $LedgerSheetNoSalesProdList->getFormListData($searchArray);
        ...
        private function initialDisplay($mode)
        {
            ...
            $abbreviatedNameList = $LedgerSheetNoSalesProdList->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
            $modal = new Modal();
            $org_id_list    = [];
            ...
            if($mode != "initial") {
                ...
                // 帳票フォーム一覧データ取得
                $ledgerSheetDetailList = $LedgerSheetNoSalesProdList->getFormListData($searchArray);
 ...
 */
            ...
            {
                require_once './View/LedgerSheetNoSalesProdListPanel.html';
            }
        }
        ...
        public function csvoutputAction()
        {
            ...
            array_push($export_csv_title, "ゼロ期間");
            array_push($export_csv_title, "最終販売日");
            ...
            if( touch($file_path) ){
                ...
                foreach( $ledgerSheetDetailList as $rows ){
                    ...
                    $str = $str.'","'.round($rows['zero_duration'],0);
                    $str = $str.'","'.$rows['last_sale_date'];
                    $str = $str.'"';
            ...
            header("Content-Disposition: attachment; filename=ゼロ商品一覧表".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
        ...
        private function changeHeaderItemMark( $sortNo )
        {
            ...
            $headerArray = array(
                ...
                'prod_nmSortMark'           => '',
                'prod_knSortMark'           => '',
                'zero_durationSortMark'     => '',
                'last_sale_dateSortMark'    => '',
            );
            if(!empty($sortNo))
            {
                ...
                $sortList[9]    = "prod_knSortMark";
                $sortList[10]   = "prod_knSortMark";
                $sortList[11]   = "zero_durationSortMark";
                $sortList[12]   = "zero_durationSortMark";
                $sortList[13]   = "last_sale_dateSortMark";
                $sortList[14]   = "last_sale_dateSortMark";
