<?php
    ...
    // コスト帳票処理モデル
    require './Model/LedgerSheetProductResultstest.php';
     ...
     */
    class LedgerSheetProductResultstestController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        public function datasearchAction()
        {
            global $Log; // グローバル変数宣言
            $Log->trace("START showAction");            
            $ledgerSheetProductResultstest = new LedgerSheetProductResultstest();

            $searchparam = json_decode($_POST['param'],true);
            //print_r($searchparam);
            
           $result = $ledgerSheetProductResultstest->get_data_1($searchparam);
 ...
 /**
        ...
        public function csvoutputAction()
        {
            ...
            $file_path = str_replace("/", "",parent::escStr( $startDateM )).".csv";
            ...
            header("Content-Disposition: attachment; filename=商品別実績".basename($file_path).";" );
        ...
        private function initialDisplay($mode)
        {
            ...
            // 帳票フォーム一覧データ取得
            $data           = [];
            //$data           = $ledgerSheetProductResultstest->get_data();
            $data           = $ledgerSheetProductResultstest->get_data_1($searchparam);
            $org_id_list    = [];
            ...
            $appoprod_detail = $modal->getappoprod_detail();
            $spe_sales_lst  = [];
            $spe_sales_lst  = $ledgerSheetProductResultstest->getspe_sales_list();
            $spe_sales_detail = [];
            $spe_sales_detail = $ledgerSheetProductResultstest->getspe_sales_detail();
            $Sizelist       = [];
            $base           = new BaseModel();
            ...
            require_once './View/LedgerSheetProductResultstestPanel.html';
