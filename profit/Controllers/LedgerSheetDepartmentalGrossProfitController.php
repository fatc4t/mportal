<?php
     ...
     * @date      2018/08/30
     * @version   1.00
     * @note      帳票 - 部門別粗利速報の閲覧を行う
     */

    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // Login処理モデル
    require './Model/LedgerSheetDepartmentalGrossProfit.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetDepartmentalGrossProfitController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        public function pdfoutputAction()
        {
            ...
            $ledgerSheetDepartmentalGrossProfit = new ledgerSheetDepartmentalGrossProfit();
            ...
            //テスト用
            //$html = file_get_contents("./View/LedgerSheetDepartmentalGrossProfitPDFPanel.html");
            ...
            $list = $ledgerSheetDepartmentalGrossProfit->getFormListData($searchArray);
        ...
        private function initialDisplay($mode)
        {
           ...
           // print_r($_POST);
            ...
            $abbreviatedNameList = $ledgerSheetDepartmentalGrossProfit->setPulldown->getSearchOrganizationList( 'reference' );      // 組織略称名リスト
            $modal = new Modal();
            $org_id_list    = [];
            ...
            if($mode != "initial") {
                ...
                // 帳票フォーム一覧データ取得
                $ledgerSheetDetailList = $ledgerSheetDepartmentalGrossProfit->getFormListData($searchArray);
...
//                    array_push($ledgerSheetDetailList, $sumLine);
            ...
            {
                require_once './View/LedgerSheetDepartmentalGrossProfitPanel.html';
            }
        }
...
//                            $str = $str.'","'.$rows['department_code'];
                    ...
                    if($rows['sale_total'] == 0 )
                    {
                            $str = $str.'","0.00';
                    }else{
                            $str = $str.'","'.round($rows['sale_profit'] / $rows['sale_total'] * 100,2);
                    }
                    $str = $str.'"';
            ...
            header("Content-Disposition: attachment; filename=部門別粗利速報".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
        ...
        private function changeHeaderItemMark( $sortNo )
        {
            ...
            if(!empty($sortNo))
            {
                ...
                $sortList[3]    = "type_cdSortMark";
                $sortList[4]    = "type_cdSortMark";
                $sortList[5]    = "type_nmSortMark";
                $sortList[6]    = "type_nmSortMark";
                $sortList[7]    = "sect_cdSortMark";
                $sortList[8]    = "sect_cdSortMark";
                $sortList[9]    = "sect_nmSortMark";
                $sortList[10]   = "sect_nmSortMark";
                $sortList[11]   = "sale_amountSortMark";
                $sortList[12]   = "sale_amountSortMark";
                $sortList[13]   = "sale_totalSortMark";
                $sortList[14]   = "sale_totalSortMark";
                $sortList[15]   = "sale_profitSortMark";
                $sortList[16]   = "sale_profitSortMark";
                $sortList[17]   = "arariritsuSortMark";
                $sortList[18]   = "arariritsuSortMark";
