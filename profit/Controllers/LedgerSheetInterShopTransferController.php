<?php
    /**
     * @file      帳票 - 店間移動明細表コントローラ
     * @author    millionet bhattarai
     * @date      2020/02/05
     * @version   1.00
     * @note      帳票 - 店間移動明細表の閲覧を行う
     */
    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // Login処理モデル
    require './Model/LedgerSheetInterShopTransfer.php';

    /**
     * 表示項目設定コントローラクラス
     * @note   表示項目設定の新規登録/修正/削除を行う
     */
    class LedgerSheetInterShopTransferController extends BaseController
    {
        /**
         * コンストラクタ
         */
        ...
        private function initialDisplay($mode)
        {
            ...
            $LedgerSheetInterShopTransfer = new LedgerSheetInterShopTransfer();
            //モーダル
            $modal = new Modal();
            //移動元指定モーダル取得 
            $org_id_list    = [];
            ...
            //店移動先指定モーダル取得 
            $supp_detail    = [];
            $supp_detail    = $modal->getInSupp_detail();
            //商品指定モーダル取得
            $prod_detail    = [];
            ...
            require_once './View/LedgerSheetInterShopTransfer.html';
        ...
        public function getdataAction($mode)
        {
            ...
            $LedgerSheetInterShopTransfer = new LedgerSheetInterShopTransfer();
            $data['dataget'] = $LedgerSheetInterShopTransfer->get_data($joken);
 ...
 /**
        ...
        public function csvoutputAction()
        {
            ...
            header("Content-Disposition: attachment; filename=店間移動明細表".mb_convert_encoding(basename($file_path), 'SJIS-win', 'UTF-8').";" );
