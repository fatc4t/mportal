<?php
     ...
     */
    class LedgerSheetSupplierTotal extends BaseModel
    {
        /**
         * コンストラクタ
         */
        ...
        private function creatSQL( $postArray, &$searchArray )
        {
            ...
            {
                $where .= ' where date(b.stock_date) >= date(:start_date) ';
            ...
            {
                $where .= ' AND date(b.stock_date) <= date(:end_date) ';
            ...
            if( !empty( $postArray['supp_cd_start'] ) )
            {
                $where .= ' AND b.supp_cd >= :supp_cd_start ';
                $ledgerSheetFormIdArray = array(':supp_cd_start' => $postArray['supp_cd_start'],);
                $searchArray = array_merge($searchArray, $ledgerSheetFormIdArray);
            }

            if( !empty( $postArray['supp_cd_end'] ) )
            {
                $where .= ' AND b.supp_cd <= :supp_cd_end ';
                $ledgerSheetFormIdArray = array(':supp_cd_end' => $postArray['supp_cd_end'],);
                $searchArray = array_merge($searchArray, $ledgerSheetFormIdArray);
            }
            if( !empty( $postArray['supplier'] ) )
            {
                $where .= ' AND x.supp_nm ~ :supplier ';
                $ledgerSheetFormIdArray = array(':supplier' => $postArray['supplier'],);
                $searchArray = array_merge($searchArray, $ledgerSheetFormIdArray);
            }

            $sql  = " SELECT DISTINCT
                    b.supp_cd AS supp_cd,
                    MAX(x.supp_nm) AS supp_nm,
                    SUM(a.amount) AS amount,
                    SUM(a.amount*a.saleprice) AS saleprice,
                    SUM(a.costtotal) AS costtotal
                    FROM trn1102 a
                    JOIN trn1101 b ON a.hideseq = b.hideseq
                    JOIN mst1101 x ON b.supp_cd = x.supp_cd  ";
            $sql .= $where;
            $sql .= "   GROUP BY b.supp_cd";
            $sql .= "   ORDER BY b.supp_cd asc  ";
