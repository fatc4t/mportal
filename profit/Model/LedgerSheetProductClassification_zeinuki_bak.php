<?php
    ...
    class LedgerSheetProductClassification_zeinuki extends BaseModel
    {
        ...
        public function getFormListData( $postArray, &$searchArray )
        {
            ...
            $sql1  .= "  , sum(t.genkin_uitem_calc+t.genkin_iitem_calc+t.genkin_hitem_calc+ ";
            $sql1  .= "        t.kake_uitem_calc+t.kake_iitem_calc+t.kake_hitem_calc) as sale_total ";
            ...
            $sql1  .= "  			 else t_data.genkin + t_data.point_money- round(t_data.itax * (t_data.genkin + t_data.point_money ) / NULLIF(t_data.iitem + t_data.itax,0))  end ,0) as genkin_iitem_calc 	 ";
            ...
            $sql1  .= "        			, case when (azukari <> 0   and turisen >= 0)  or ( azukari<0 and turisen <0) then genkin else 0 end as genkin ";
            $sql1  .= " 				, case when genkin + point_money <> 0 then t3.profit_i else 0 end as genkin_saleprofit ";
            $sql1  .= " 				, case when genkin + point_money  <> 0 then t3.amount   else 0 end as genkin_amount ";
            $sql1  .= " 				, case when credit_money+gift_money <> 0 then t3.amount else 0 end as kake_amount ";
            $sql1  .= " 				, case when credit_money+gift_money <> 0 then t3.profit_i else 0  end as kake_saleprofit ";
            $sql1  .= " 				, case when ( ";
            $sql1  .= " 				 (case when genkin is null then 0 else (genkin + point_money) end) + ";
            $sql1  .= " 				 (case when credit_money is null then 0 else credit_money end)) = 0 then 0 else ( ";
            $sql1  .= " 				 (case when genkin + point_money <> 0 then t3.profit_i else 0 end ) + ";
            $sql1  .= " 				 (case when credit_money+gift_money <> 0 then t3.profit_i else 0 end)) / ( ";
            $sql1  .= " 				 (case when genkin is null then 0 else (genkin + point_money) end) + ";
            $sql1  .= " 				 (case when credit_money is null then 0 else credit_money end)) * 100 end as sale_margin  ";
            $sql1  .= " 				,(case when genkin + point_money <> 0 then t3.amount else 0 end) + ";
            $sql1  .= " 				 (case when credit_money+gift_money <> 0 then t3.amount else 0 end) as sale_amount ";
            $sql1  .= " 				,(case when genkin is null then 0 else (genkin + point_money) end) + ";
            $sql1  .= " 				 (case when credit_money is null then 0 else (credit_money + gift_money) end) as sale_total  ";
            $sql1  .= " 				,(case when genkin + point_money <> 0 then t3.profit_i else 0 end) + (case when credit_money <> 0 then t3.profit_i else 0 end) as sale_profit  ";
            $sql1  .= "  				, credit_money  ";
            $sql1  .= "  				, gift_money ";
            $sql1  .= "  				, point_money ";
            ...
            $sql1  .= "  					   from trn0102 t2 	 ";
            $sql1  .= "  					   left join trn0101 t1 using (organization_id,hideseq) 	 ";
            $sql1  .= "  					   where cancel_kbn = '0' and stop_kbn = '0'  ";	
 ...
 /*           $sql1  .= "select ";
            ...
            $sql1  .= "  , sum(t.sale_margin) as sale_margin ";
            $sql1  .= "  , sum(t.sale_amount) as sale_amount ";
            $sql1  .= "  , sum(t.sale_total - t.tax_total) as sale_total  ";
            $sql1  .= "  , sum(t.sale_profit) as sale_profit ";
            $sql1  .= "  , sum(t.genkin_pure_total-(t.genkin_utax+t.genkin_itax))as genkin_pure_total ";
            $sql1  .= "  , sum(t.kake_pure_total -(t.kake_utax+t.kake_itax))as kake_pure_total ";
            $sql1  .= "from ";
            $sql1  .= "  (  ";
            $sql1  .= "    select ";
            $sql1  .= "      coalesce(case when (t3.genkin +t3.point_money <> 0) then (t1.itax) else 0 end ,0) as genkin_itax ";
            $sql1  .= "      ,coalesce(case when (t1.uitem <> 0 and (t1.iitem <> 0 or t1.hitem <> 0) and (t3.genkin + t3.point_money) <> 0) "; 
            $sql1  .= "           then round(t1.utax*(t3.genkin + t3.point_money - t1.iitem - t1.itax - t1.hitem)/NULLIF(t1.uitem + t1.utax,0)) ";
            $sql1  .= "           else round(t1.utax*(t3.genkin + t3.point_money) / NULLIF(t1.uitem + t1.utax,0)) end,0) as genkin_utax ";
            $sql1  .= "      ,coalesce(case when (t1.iitem <> 0 and(t1.uitem <> 0 or t1.hitem <> 0) and (t3.credit_money+t3.gift_money) <> 0) "; 
            $sql1  .= "           then round(t1.itax*(t3.credit_money+t3.gift_money - t1.uitem - t1.utax - t1.hitem) / NULLIF(t1.iitem + t1.itax,0)) ";
            $sql1  .= "           else round(t1.itax*(t3.credit_money+t3.gift_money) / NULLIF(t1.iitem + t1.itax,0)) end ,0) as kake_itax ";
            $sql1  .= "      ,coalesce(case when (t1.uitem <> 0 and(t1.iitem <> 0 or t1.hitem <> 0) and (t3.credit_money) <> 0)  ";
            $sql1  .= "           then round(t1.utax*(t3.credit_money+t3.gift_money - t1.iitem - t1.itax - t1.hitem) / NULLIF(t1.uitem + t1.utax,0)) ";
            $sql1  .= "           else round(t1.utax*(t3.credit_money+t3.gift_money)/ NULLIF(t1.uitem + t1.utax,0)) end,0)as kake_utax ";
            $sql1  .= "      ,coalesce(case when t2.tax_type = '9' then t2.itax else 0 end, 0) as t_htax ";
            $sql1  .= "	     , t1.utax + t1.itax as tax_total  ";
            ...
            $sql1  .= "      ,case when (  ";
            $sql1  .= "       (case when t3.genkin is null then 0 else (t3.genkin + t3.point_money) end) +  ";
            $sql1  .= "       (case when t3.credit_money is null then 0 else t3.credit_money end)) = 0 then 0 else (  ";
            $sql1  .= "       (case when t3.genkin > 0 then t2.profit_i else 0 end ) +  ";
            $sql1  .= "       (case when t3.genkin = 0 then t2.profit_i else 0 end)) / (  ";
            $sql1  .= "       (case when t3.genkin is null then 0 else (t3.genkin + t3.point_money) end) +  ";
            $sql1  .= "       (case when t3.credit_money is null then 0 else t3.credit_money end)) * 100 end as sale_margin ";
            $sql1  .= "      ,(case when t3.genkin > 0 then t2.amount else 0 end) +  ";
            $sql1  .= "       (case when t3.genkin = 0 then t2.amount else 0 end) as sale_amount ";
            $sql1  .= "      ,(case when t3.genkin is null then 0 else (t3.genkin + t3.point_money) end) +  ";
            $sql1  .= "       (case when t3.credit_money is null then 0 else (t3.credit_money + t3.gift_money) end) as sale_total ";
            ...
            $sql1  .= "    order by ";
            $sql1  .= "      t1.proc_date, t1.organization_id ";
            $sql1  .= "  ) t ";
            $sql1  .= "group by ";
            $sql1  .= "  t.prod_t_cd1, t.prod_t_nm_1  ";
            if($postArray['middle_class_chk']){
