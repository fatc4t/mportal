<?php
     ...
     */
    class SalesReportListDay extends BaseModel
    {
        /**
         * コンストラクタ
         */
         ...
         * @return   成功時：$displayItemList(帳票フォームデータ)  失敗：無
         */
        public function getTotalDeta( $postArray )
        {
            global $DBA, $Log; // グローバル変数宣言
            $Log->trace("START getTotalDeta");
           
            $searchArray = array();
            
            $sql = " SELECT SUM(to_number(data,'999999999')) as data"
                 . "   FROM t_report_data_day"
                 . "  WHERE target_date >= DATE_TRUNC('month', date(:target_date))"
            ...
            {
                $reportFormIdArray = array(':report_form_detail_id' => $postArray['report_form_detail_id'],);
                $searchArray = array_merge($searchArray, $reportFormIdArray);
            }
