<?php
     ...
     */
    class LedgerSheetHourlySales extends BaseModel
    {
        /**
         * コンストラクタ
         */
        ...
        private function creatSQL( $postArray, &$searchArray )
        {
           ...
           if( !empty( $postArray['start_date'] ) )
            {
                $where .= ' AND date(a.proc_date) >= :start_date ';
            ...
            {
                $where .= ' AND date(a.proc_date) <= :end_date ';
            ...
            if(isset($_POST['hi'])){
            $hi = ' proc_date, ';
            $groupby = " GROUP BY proc_date, a.tmzone_cd, b.tmzone_str, b.tmzone_end 
                         ORDER BY proc_date, tmzone_cd ";
            }
            else{
            $hi = '';
            $groupby = " GROUP BY a.tmzone_cd, b.tmzone_str, b.tmzone_end 
                         ORDER BY tmzone_cd  ";
            }
            $sql  = " SELECT ";
            $sql .= $hi; 
            $sql .= " a.tmzone_cd, 
                    ...
                    SUM(pure_total) pure_total, 
                    SUM(sale_amount) sale_amount, 
                    SUM(sale_profit) sale_profit,
                    b.tmzone_str,
                    b.tmzone_end
                    
                    FROM jsk1190 a
                    JOIN mst8101 b ON a.tmzone_cd = b.tmzone_cd and a.organization_id = b.organization_id
