<?php
     ...
     */
    class LedgerSheetTimedSaleRecord extends BaseModel
    {
        /**
         * コンストラクタ
         */
        ...
        private function creatSQL( $postArray, &$searchArray )
        {
            ...
            $where = ' AND a.cancel_kbn != $$1$$ AND c.stop_kbn != $$1$$ AND c.lump_return_kbn != $$1$$ 
                    AND c.return_hideseq != a.hideseq  ';
           if( !empty( $postArray['start_date'] ) )
            ...
            $sql  = "  SELECT DISTINCT
                    ...
                    MAX(CASE WHEN
                        b.prod_nm = '' THEN (SELECT b.prod_kn)
                        ELSE (SELECT b.prod_nm)
                    END) AS prod_nm,
                    (CASE WHEN
                        b.prod_capa_nm = '' THEN (SELECT prod_capa_kn)
                        ELSE (SELECT prod_capa_nm)
                    END) AS prod_capa,
                    a.sale_plan_cd AS sale_plan_cd,
                    SUM(a.amount) AS amount,
                    SUM(a.subtotal) AS subtotal,
                    AVG(a.subtotal) AS avgsubtotal,
                    (CASE WHEN
                        b.tax_type = '2' THEN (SELECT b.saleprice_ex)
                        ELSE (SELECT b.saleprice)
                    END) AS saleprice,
                    (CASE WHEN
                        b.tax_type = '2' THEN (SELECT AVG(b.saleprice_ex))
                        ELSE (SELECT AVG(b.saleprice))
                    END) AS avgsaleprice
                    FROM
                    trn0102 a
                    JOIN trn0101 c ON a.hideseq = c.hideseq AND c.organization_id = a.organization_id
                    JOIN v_organization v ON v.organization_id = a.organization_id
                    JOIN mst0201 b ON a.prod_cd = b.prod_cd AND v.organization_id = b.organization_id

                    WHERE sale_plan_cd != ''

                    ";
                    $sql .= $where;
                    $sql.= "  GROUP BY a.prod_cd, b.sect_cd, prod_nm, a.sale_plan_cd, prod_capa, b.tax_type, b.saleprice_ex, b.saleprice, v.abbreviated_name ";
                    $sql .= " ORDER BY sect_cd asc, prod_cd  ";
