    ...
    class LedgerSheetProductClassification extends BaseModel
    {
        ...
        public function getFormListData( $postArray, &$searchArray )
        {
           ...
           $sql1  .= "     , t.prod_t_nm_1 ";
            ...
            $sql1  .= "  , sum(t.sale_amount) as sale_amount   ";
            $sql1  .= "  , sum(t.sale_profit) as sale_profit   ";
            $sql1  .= "  , sum(t.sale_pure_total) as sale_total ";
            $sql1  .= "  , coalesce(sum(t.sale_profit)/NULLIF(sum(t.sale_pure_total),0)*100,0) as sale_margin ";
            $sql1  .= " from ";
            $sql1  .= "  (  ";
            $sql1  .= "    select ";
            ...
            $sql1  .= "      , t_data.genkin_pure_total as genkin_pure_total  ";
            $sql1  .= "      , t_data.kake_amount as kake_amount              ";
            $sql1  .= "      , t_data.kake_saleprofit as kake_saleprofit      ";
            $sql1  .= "      , t_data.kake_pure_total as kake_pure_total      ";
            ...
            $sql1  .= "      , (t_data.kake_pure_total+t_data.genkin_pure_total) as sale_pure_total ";
            if($postArray['middle_class_chk']){
            ...
            $sql1  .= "          , coalesce(case when (azukari <> 0 and turisen >= 0) or (azukari < 0 and turisen < 0 and point_money <> 0) then t2.profit_i else 0 end , 0) as genkin_saleprofit ";
//            $sql1  .= "          , coalesce(case when (azukari <> 0 and turisen >= 0) or (azukari < 0 and turisen < 0 and point_money <> 0) then t2.pure_total else 0 end , 0) as genkin_pure_total ";
...
//            $sql1  .= "          , coalesce(case when (credit_money + gift_money <> 0) then t2.pure_total else 0 end , 0) as kake_pure_total ";
            $sql1  .= "          , (genkin +point_money)  as genkin_pure_total  ";
            $sql1  .= "          , case  when credit_money is null then 0 else  (credit_money +gift_money) end  as kake_pure_total  ";
            $sql1  .= "          , credit_money ";
            $sql1  .= "          , gift_money ";
            $sql1  .= "          , point_money ";
            $sql1  .= "          , t2.pure_total ";
            ...
            $sql1  .= "              , t2.hideseq ";
            $sql1  .= "              , sum(case when(t2.tax_type = '1') then (t2.pure_total+t2.itax)else t2.pure_total end) as pure_total ";
            ...
            if($postArray['middle_class_chk']){			
                    $sql1  .= " 	 left join mst0801 m10 on t3.organization_id = m10.organization_id and t3.prod_t_cd1 = m10.prod_t_cd1 and t3.prod_t_cd2 = m10.prod_t_cd2  ";
            }  
            ...
            $sql1  .= "     where t1.proc_date between :proc_date_start and :proc_date_end ";
            $sql1  .= " $where ";
            $sql1  .= "      ) T_data where T_data.amount <> 0 ";
            $sql1  .= "  ) t ";
            $sql1  .= "group by ";
            $sql1  .= "  t.prod_t_cd1 ";
            $sql1  .= "  ,t.prod_t_nm_1 ";
