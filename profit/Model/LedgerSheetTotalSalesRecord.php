<?php
     ...
     */
    class LedgerSheetTotalSalesRecord extends BaseModel
    {
        /**
         * コンストラクタ
         */
        ...
        private function creatSQL( $postArray, &$searchArray )
        {
           ...
           if( !empty( $postArray['start_date'] ) )
            {
                $where .= ' AND date(proc_date) >= :start_date ';
            ...
            {
                $where .= ' AND date(proc_date) <= :end_date ';
            ...
            {$ten=" v.abbreviated_name AS abbreviated_name, ";}
            else{$ten='';}
            if(isset($_POST['tenpo']))
            {$groupby = " GROUP BY proc_date, a.organization_id, abbreviated_name
                          ORDER BY proc_date, a.organization_id asc   ";}
            else{$groupby= " GROUP BY proc_date ORDER BY proc_date asc ";}

            $sql  = " SELECT " ;
            $sql .= $ten;                   
            $sql .= " date(proc_date) AS proc_date,
                    SUM(total_total) AS total_total,
                    SUM(total_profit_i) AS total_profit_i,
                    SUM(total_amount) AS total_amount,
                    SUM(return_amount) AS return_amount,
                    SUM(return_total) AS return_total,
                    SUM(disc_total+perc_total+disc_item_total+perc_item_total+adjust_total) AS disc_total,
                    SUM(pure_total_i) AS pure_total_i,
                    SUM(total_utax+total_itax) AS total_tax                    
