<?php
     ...
     */
    class LedgerSheetDailySalesResults extends BaseModel
    {
        /**
         * コンストラクタ
         */
 ...
 /*           
            $sql  = "";
            $sql .= " select j.organization_id  as org_id " ;
            ...
            $sql .= " 	end,2) as res4 " ;
            $sql .= " 	,-1*sum(j.return_total) as return_total " ;
            $sql .= " 	,-1*sum(j.return_amount) as return_amount	 " ;
            $sql .= " from jsk1010 j " ;
  ...
  */
            ...
            $sql .= " 		round(case sum(a.total_amount) when 0 then 0 else round(sum(a.pure_total_i)/ sum(a.total_amount), 2) end, 2) as res4 , ";
            $sql .= " 		-1 * sum(a.return_total) as return_total , ";
            $sql .= " 		-1 * sum(a.return_amount) as return_amount, ";
            ...
            $sql .= " 			sum(return_total) as return_total, ";
            $sql .= " 			sum(return_amount) as return_amount ";
            $sql .= " 		from jsk1010 ";
            $sql .= " 		group by ";
            $sql .= " 			organization_id, ";
            $sql .= " 			reji_no, ";
