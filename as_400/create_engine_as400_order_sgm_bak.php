...
try {
    ...
    $mht_youbis = get_mht_youbi($today);
    // 当日発注できる店舗コードでループを回る
    ...
    foreach ($mht_youbis as $mht_youbi) {
        ...
        $sql .= "       ,TRN1602.COSTPRICE           as COSTPRICE         "; // 原価
        $sql .= "       ,TRN1602.SALEPRICE           as SALEPRICE         "; // 売価
        $sql .= "       ,TRN1602.LINE_NO             as LINE_NO           "; // 行番号
        //    $sql .= "       ,TRN1602.SALE_PLAN_KBN       as SALE_PLAN_KBN     ";// 特売区分
        ...
        $sql .= "            END AS CLASS_CD                              ";
        //     $sql .= "       ,CASE                                             ";
        //     $sql .= "            WHEN TRN1602.DIRECT_KBN = NULL               ";// TRN1601の直送区分んが空白の場合
        $sql .= "               , MST0201.RESAlE_KBN                   "; // 商品マスタを参照する
        //     $sql .= "            ELSE TRN1602.DIRECT_KBN                      ";// TRN1602空白でない場合は
        ...
        if ($rows_import) {
            ...
            foreach ($rows_import as $row_import) {
                ...
                // 特売マスタを参照
                $m_tokubai = get_m_tokubai($schema, $prod_cd, $ord_date);
                // 曜日マスタを取得
                    ...
                    if ($strdata_set_kbn === '1') {
                        ...
                        if ($str_send_sale_kbn === '1') {
                            ...
                            if (count($m_tokubai) <= 0) {
                                ...
                                // TRNデータの原価を採用する
                                $strCostprice = sprintf("%06d", intval($row_import['saleprice']));
                            } else {

                                // 特売マスタの原価採用する
                                $strCostprice = sprintf("%06d", intval($m_tokubai['saleprice1']));
                            }
                        } else {
                            //　送信しない
                            $strSaleprice = str_pad('', 6);
                ...
                // 本日を取得（ｙｙｙｙ-ｍｍ-ｄｄ）
                $Dt_today   = date('Y-m-d');
                // $Dt_today = date('2021-08-11');
                //　本日を納品日と同じ形にする（ｙｙ-ｍｍ-ｄｄ）
                $today = substr($Dt_today, 2, 8);
                // 過去の納品日と本日納品日の場合、+2日する
                if($A_arr_date <= $today){
                     $A_arr_date = date('Y-m-d', strtotime($Dt_today .'+2 day'));
                }
...
error_log('END', 3, '/var/www/as_400/order/SGM.dat');
