<?php
    /**
     * @file    テロップ 初期化用クラス(Model)
     * @author  oota
     * @date    2017/02/02
     * @version 1.00
     * @note    テロップで使用する初期データの設定について定義する
     */
     ...
     */
    class SetTelopInitialization extends FwBaseClass
    {
        // DBアクセスクラス
         ...
         * @return   無
         */
        public function setTelopInit()
        {
            global $Log; // グローバル変数宣言
            $Log->trace("START setTelopInit");

            // 管理機能用のメニューリスト設定
            $this->setMenuList();
            // マスタごとの参照テーブル一覧設定
            $_SESSION["TELOP_ACCESS_AUTHORITY_TABLE"] = $this->getAccessAuthorityTableList();

            $Log->trace("END setTelopInit");
        }
        ...
        private function setMenuList()
        {
            ...
            $_SESSION["TELOP_TOP_MENU"]        = array(
                                                    SystemParameters::$V_TELOP_HOME                    => "テロップ",
                                                );
        ...
        private function getAccessAuthorityTableList()
        {
            ...
            $ret = array(
                            SystemParameters::$V_TELOP_HOME             => "m_telop",  // テロップテーブル
                        );
