<?php
     ...
     */
    class SetNoticeContactInitialization extends FwBaseClass
    {
        // DBアクセスクラス
         ...
         * @return   無
         */
        public function setNoticeContactInit()
        {
            global $Log; // グローバル変数宣言
            $Log->trace("START setNoticeContactInit");

            // 管理機能用のメニューリスト設定
            $this->setMenuList();
            // マスタごとの参照テーブル一覧設定
            $_SESSION["NC_ACCESS_AUTHORITY_TABLE"] = $this->getAccessAuthorityTableList();

            $Log->trace("END setNoticeContactInit");
        }
        ...
        private function setMenuList()
        {
            ...
            $_SESSION["NC_TOP_MENU"]        = array(
                                                    SystemParameters::$V_NC_HOME                    => "通達連絡",
                                                );
        ...
        private function getAccessAuthorityTableList()
        {
            ...
            $ret = array(
                            SystemParameters::$V_NC_HOME             => "t_notice_contact",  // トップメッセージテーブル
                        );
