<?php
     ...
     */
    class SetDailyReportInitialization extends FwBaseClass
    {
        // DBアクセスクラス
         ...
         * @return   無
         */
        public function setDailyReportInit()
        {
            global $Log; // グローバル変数宣言
            $Log->trace("START setDailyReportInit");

            // 管理機能用のメニューリスト設定
            $this->setMenuList();
            // マスタごとの参照テーブル一覧設定
            $_SESSION["DRM_ACCESS_AUTHORITY_TABLE"] = $this->getAccessAuthorityTableList();

            $Log->trace("END setDailyReportInit");
        }
        ...
        private function setMenuList()
        {
            ...
            $_SESSION["DRM_TOP_MENU"]        = array(
                                                    SystemParameters::$V_DR_HOME                    => "日報",
                                                );
        ...
        private function getAccessAuthorityTableList()
        {
            ...
            $ret = array(
                            SystemParameters::$V_DR_HOME             => "t_daily_report",  // トップメッセージテーブル
                        );
