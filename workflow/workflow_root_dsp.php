...
//echo "&nbsp;CC番目：".$cc_no."<br />\n";
		...
		if ($chg_date == ""){	//上書き
			if ($kengen == 1){		//承認者
				...
				$sql = "update ".$schema.".m_workflow set cc".$cc_no." = '".$user_list."-2' where target_date = '".$target_date2."' and busho_code = ".$chg_org." and flow_doc_code = ".$chg_flow;
				sqlExec($sql);
			}
		}else{	//新規追加
			if ($kengen == 1){		//承認者
				...
				$sql = "update ".$schema.".m_workflow set cc".$cc_no." = '".$user_list."-2', target_date = '".$chg_date."' where m_workflow_id = '".$last_id."'";
				sqlExec($sql);
			}
		}
...
//echo "&nbsp;適用日：".$chg_date."<br />\n";
	...
	if ($hani == 1){	//1対象ルート指定削除
		...
		sqlExec($sql);
			...
			if ($kengen == 1){		//承認者
				workflow_root_target_delete_shounin($schema, $target_date2, $chg_flow, $chg_org, $target_staff, $kengen_no);
			}else if ($kengen == 2){	//CC
...
//				      	$workflow_html .= "<th colspan=4>".code_name_dsp(0, org_id_code_get($row_org["organization_detail_id"]))."&nbsp;&nbsp;&nbsp;".f_code_dsp(99, $row_mcode["no"])."&nbsp;&nbsp;(<a href='workflow_root_dsp.php?up=3&target_date=".$target_date."&target_flow=".$target_flow."&target_org=".$target_org."&chg_flow=".$row_mcode["no"]."&target_date2=".$target_date2."&chg_org=".$row_org["organization_detail_id"]."'>ルートを作る</a>)</th>\n";
	...
	goto skip2;
					...
					while($f_rows = $rows[$cnt]) {
						...
						$cc1 = workflow_kian_cc_staff($f_rows["cc1"]);
						$cc2 = workflow_kian_cc_staff($f_rows["cc2"]);
						$cc3 = workflow_kian_cc_staff($f_rows["cc3"]);
						$cc4 = workflow_kian_cc_staff($f_rows["cc4"]);
						$cc5 = workflow_kian_cc_staff($f_rows["cc5"]);
						for ($i = 1; $i <= 10; $i++){
...
//					      	$workflow_html .= "<th colspan=4>".code_name_dsp(1, $f_rows["busho_code"])."&nbsp;&nbsp;&nbsp;".code_name_dsp(0, org_id_code_get($f_rows["busho_code"]))."&nbsp;&nbsp;&nbsp;".f_code_dsp(99, $f_rows["flow_doc_code"])."&nbsp;&nbsp;(適用年月日：".$target_date2.")</th>\n";
						...
						for ($i=1; $i <= $next_count; $i++){
							...
							$workflow_html .= "<tr>\n";
						      	$workflow_html .= "<td align=center rowspan=1>".($i+1)."</td>\n";
			//複数承認者の名前を列記する
						      	$workflow_html .= "<td align=left rowspan=1>".$workflow_data[$i][0][0]."</td>\n";
			//			      	$workflow_html .= "<td align=left rowspan=".$workflow_cc_data[$i].">".f_staff_name_get($workflow_data[$i][0][0])."</td>\n";
			//承認人数を表示
						      	$workflow_html .= "<td align=center rowspan=1>".$workflow_data[$i][0][3]." / ".$workflow_data[$i][0][2]."</td>\n";
