...
{
	$sql = "SELECT * FROM ".$schema.".m_workflow WHERE (cc1 = '".$target_staff."-2' or cc2 = '".$target_staff."-2' or cc3 = '".$target_staff."-2' or cc4 = '".$target_staff."-2' or cc5 = '".$target_staff."-2') and ".$where;
        $rows = getList($sql);
	$cnt = 0;
        if($rows){
		while($row = $rows[$cnt]) {
			for ($i = 1; $i <= 10; $i++){
				if($row["cc".$i] == $target_staff."-2"){
					$sql_up = "update ".$schema.".m_workflow set cc".$i." = '".$user_list."-2' where m_workflow_id = '".$row["m_workflow_id"]."'";
					sqlExec($sql_up);
				}
			}
			$cnt += 1;
		...
		while($row = $rows[$cnt]) {
			for ($i = 1; $i <= 10; $i++){
				if($row["cc".$i] == $target_staff."-2"){
					...
					$sql_up = "update ".$schema.".m_workflow set cc".$i." = '".$user_list."-2', target_date = '".$chg_date."' where m_workflow_id = '".$last_id."'";
					sqlExec($sql_up);
				}
			}
			$cnt += 1;
