						...
						case 10:	//input
							switch($data12)
							{
								...
								case 4:	//type = dsp 表示のみ
									if ($data16 != "none"){
										$csv_data[$csv_data_cnt] .= $data2.",".$data16.",";
									}else{
										$csv_data[$csv_data_cnt] .= $data2.",".",";
									}
									break;
...
//			$workflow_html .= "<th align=center class=\"title_td\" width=80>結果報告</th>\n";
			...
			while($row = $f_rows[$cnt]) {
				...
				$workflow_html .= "<tr>\n";
			      	...
			      	$workflow_html .= "<td align=center class=\"item_td\"><a href=\"./apply_view.php?home=".$home."&cd=1&page=".$page."&workflow_id=".$row["workflow_id"]."\">".$row["workflow_no"]."</a></td>\n";
...
//	if (f_staff_super_get($login_staff) == 1){
//		$sql_page = "SELECT * FROM ".$schema.".t_workflow ORDER BY workflow_id DESC ";
//	}else{
...
//	}
	...
	$pageList = pageListCreate($page_max, "page", $sql_page, $page, "apply_view.php?home=".$home."&cd=0&stat=".$k_stat."&apply_no=".$apply_no."&apply_org=".$apply_org."&flow_doc=".$flow_doc."&apply_title=".$apply_title."&apply_body=".$apply_body);
...
<?php
			...
			echo "<tbody>\n";
			echo "<tr>\n";
			echo "<th>申請番号</th><td><input type='text' id='apply_no' name='apply_no' size=5 value=".$apply_no."></td>\n";
