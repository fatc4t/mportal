...
function workflow_no($schema, $busho_code, $staff_code, $yyyy, $mm, $dd)
{
	...
	if ($num == 0){
			...
			$rows = $f_rows[0];
			$sirial_no = $rows["no"] + 1;
			$sql = "UPDATE ".$schema.".t_workflow_no SET no = ".$sirial_no." where t_workflow_no_id = ".$rows["no"];
			sqlExec($sql);
	}
...
function workflow_doc_inf($cd, $id, $staff_code)
{
	...
	if($f_rows){
		...
		$workflow_html .= "<th colspan=2>タイトル</th>\n";
	      	$workflow_html .= "<td align=left>".$f_rows["title"]."</td>\n";
		$workflow_html .= "</tr>\n";
/*
		$workflow_html .= "<tr>\n";
			...
			$cnt_remand = 0;
				...
				while($row_remand = $rows_remand[$cnt_remand]) {
					if ($row_remand["workflow_id"] != $id){
						...
						$workflow_html .= "<td><a href='workflow_dsp.php?workflow_id=".$row_remand["workflow_id"]."' target=_blank>".$row_remand["title"]."</a></td>\n";
						$workflow_html .= "</tr>\n";
					}
					$cnt_remand += 1;
				}
...
//ここまで
	...
	if ($next_count == 0){
			...
			if ($cc2 != "0"){
				$cc .= "<br/>".f_staff_name_get($cc2);
			}
			if ($cc3 != "0"){
				$cc .= "<br/>".f_staff_name_get($cc3);
			}
			if ($cc4 != "0"){
				$cc .= "<br/>".f_staff_name_get($cc4);
			}
			if ($cc5 != "0"){
				$cc .= "<br/>".f_staff_name_get($cc5);
			}
...
//		$workflow2_html .= "<a href=\"#\" onclick=\"stop()\"><span class=\"undo\" /></span>取り戻す</a>\n";
	...
	if (($status == 2 || $status == 3 || $status == 4) && $input_user == $staff_code_tmp){	//再利用ボタンを表示
		$workflow2_html .= "<br /><p align=center><input type=\"button\" onclick=\"location.href='apply.php?workflow_id=".$id."'\" value=\"再利用\" class=\"confirm\" />\n";
		$f = 1;
					...
					case 30:	//date
						...
						switch($data12)
						{
							case 1:	//年
								...
								break;
							case 2:	//年月
								$script_html .= "$('#".$data8."').datepicker({dateFormat:'yy-mm'});\n";
				...
				while($row = $rows[$cnt]) {
					...
					switch($data1)
					{
						...
						case 10:	//input
							...
							break;
						case 20:	//textarea
							$html .= nl2br(f_post_name_data_get($workflow_id, $data10));
							break;
						case 30:	//date
							$html .= f_post_name_data_get($workflow_id, $data10);
							break;
...
//重要　全員の承認が終わって、かつ、必要人数に達したら、CCへ
				...
				//承認後の参照者がいるので見つけて2に変えてメールを送る
									if ($f_rows_next["next".$i."_cc".$k."_data"] != ""){
										...
										workflow_send_mail(2, $mail_address, $cc_id, $workflow_id, $flow_doc_id);
									}
								}
							}
	...
	//								$sql_up = "UPDATE ".$_SESSION["SCHEMA"].".t_workflow SET next".($i+1)."_data = '".$terget_id."-2' WHERE workflow_id = ".$workflow_id;
									...
									//承認依頼メールを送る
									workflow_send_mail(0, $mail_address, $cc_id, $workflow_id, $flow_doc_id);
								}
							}else{
				//次の承認者がいなければstatを3可決に変えて申請者にメールを送る
								...
								workflow_send_mail(1, $mail_address, $input_staff, $workflow_id, $input_flow_doc_id);
							}
							break;
							...
							//否決メールを送る
							$mail_address = f_staff_mail_get($input_staff);
							workflow_send_mail(3, $mail_address, $input_staff, $workflow_id, $input_flow_doc_id);
							...
							//否決メールを送る
							$mail_address = f_staff_mail_get($input_staff);
							workflow_send_mail(5, $mail_address, $input_staff, $workflow_id, $input_flow_doc_id);
