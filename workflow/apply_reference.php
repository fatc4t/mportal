...
//CCのターゲットを探すために　%-3　だけではダメ。全員のチェック必要
//$sql .= "(next1_data LIKE '%-3%' and (next1_cc1_data = '".$staff_code."-2' or next1_cc2_data = '".$staff_code."-2' or next1_cc3_data = '".$staff_code."-2' or next1_cc4_data = '".$staff_code."-2' or next1_cc5_data = '".$staff_code."-2')) or ";
//next1_data LIKE '%-3%' ←これが複数人に対応していない
...
/*
				$cc_ok = 1;
				if ($jj != 0){	//CC向けの承認済みチェック
					$cc_ok = workflow_cc_ok($f_row, $staff_code);
				}
...
*/
				...
				$workflow_html .= "<tr>\n";
			      	...
			      	$workflow_html .= "<td align=center class=\"item_td\"><a href=\"./apply_reference.php?home=".$home."&cd=1&workflow_id=".$f_row["workflow_id"]."\">".$f_row["workflow_no"]."</a></td>\n";
...
/*
				$str = mb_strimwidth($f_row["comment"], 0, 200, "...");
			      	$workflow_html .= "<td align=left class=\"item_td\">".$str."</td>\n";
*/
			      	$workflow_html .= "<td align=center class=\"item_td\">参照</td>\n";
				$workflow_html .= "</tr>\n";
				$cnt += 1;
			}
		}
...
function agree(){
	...
	var target = document.getElementById("apply_reference");
	target.submit();
...
?>
		<div id="sb-site">
<?php
include("./View/Common/Breadcrumb_apply_reference.php");
?>

<form id="apply_reference" action="apply_reference.php?home=<?= $home ?>&cd=2&workflow_id=<?= $workflow_id ?>" method="post">
			<!-- serchListArea -->
