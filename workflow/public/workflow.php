...
//ここまで
	...
	if ($next_count == 0){
		...
		$workflow_html .= "</tr>\n";
			...
			$cc = "";
			if ($cc1 != "0"){
				$cc .= f_staff_name_get(str_replace("-2", "", $cc1))." ";
			}
			if ($cc2 != "0"){
				$cc .= f_staff_name_get(str_replace("-2", "", $cc2))." ";
			}
			if ($cc3 != "0"){
				$cc .= f_staff_name_get(str_replace("-2", "", $cc3))." ";
			}
			if ($cc4 != "0"){
				$cc .= f_staff_name_get(str_replace("-2", "", $cc4))." ";
			}
			if ($cc5 != "0"){
				$cc .= f_staff_name_get(str_replace("-2", "", $cc5));
			}
	...
	for ($i=1; $i <= $next_count; $i++){
		$workflow_html .= "<tr>\n";
                ...
                // rowspan=0の暫定対応 2018/04/18 oota
                if($workflow_cc_data[$i] == 0){
                    $workflow_html .= "<td align=center rowspan=1>".$i."</td>\n";
                    $workflow_html .= "<td align=center rowspan=1 cellspacing=0 cellpadding=0>\n";
                }else{
                    $workflow_html .= "<td align=center rowspan=".$workflow_cc_data[$i].">".$i."</td>\n";
		...
		for ($kk=0; $kk < $workflow_data[$i][0][4][0]; $kk++){
			...
			$workflow_html .= "</tr>\n";
	      	...
	      	$workflow_html .= "</td>\n";
                ...
                // rowspan=0の暫定対応 2018/04/18 oota
                if($workflow_cc_data[$i] == 0){
                    $workflow_html .= "<td align=center rowspan=1 width=170>".$workflow_data[$i][0][5][0]." / ".$workflow_data[$i][0][4][0]."</td>\n";
                }else{
...
function workflow_route_mobile($kyoten, $busho_code, $workflow_doc, $rank)
{
	$workflow_html = "";
	$agree_f = 0;
	$agree_c = 0;
...
//ここまで
	...
	if ($next_count == 0){
			...
			if ($cc5 != "0"){
	      	...
	      	$workflow_html .= "&nbsp;&nbsp;&nbsp;0:&nbsp;起案者<br />\n";
		$workflow_html .= "cc:&nbsp;".$cc."<br />\n";
	}
	...
	for ($i=1; $i <= $next_count; $i++){
		...
		if ($workflow_cc_data[$i] == 0){
			...
			for ($j=1; $j <= $workflow_cc_data[$i]; $j++){
			      	$workflow_html .= f_staff_name_get(str_replace("*", "", $workflow_data[$i][$j][0][0]))."&nbsp;\n";
			}
		}
	      	$workflow_html .= "<br />\n";
	}
...
//ここまで
	...
	for ($i=1; $i <= $next_count; $i++){
		$workflow_html .= "<tr>\n";
                ...
                if($workflow_cc_data[$i] == 0){
                    $workflow_html .= "<td align=center rowspan=1>".$i."</td>\n";
                    $workflow_html .= "<td align=left rowspan=1 cellspacing=0 cellpadding=0>\n";
                }else{
                    $workflow_html .= "<td align=center rowspan=".$workflow_cc_data[$i].">".$i."</td>\n";
		...
		for ($kk=0; $kk < $workflow_data[$i][0][4][0]; $kk++){
			...
			$workflow_html .= "</tr>\n";
	      	...
	      	$workflow_html .= "</td>\n";
                ...
                // rowspan=0の暫定対応 2018/04/18 oota
                if($workflow_cc_data[$i] == 0){
                    $workflow_html .= "<td align=center rowspan=1>".$workflow_data[$i][0][5][0]." / ".$workflow_data[$i][0][4][0]."</td>\n";
                }else{
...
//ここまで
	...
	$workflow_html .= "<tr>\n";
        ...
        if($workflow_cc_kian_cnt == 0){
            $workflow_html .= "<td align=center rowspan=1>0</td>\n";
            $workflow_html .= "<td align=center rowspan=1 colspan=5>起案者</td>\n";
        }else{
	...
	if ($workflow_cc_kian_cnt != 0){
		for ($i=1; $i <= $workflow_cc_kian_cnt; $i++){
					...
					if ($cd == 1 && $staff_code == "*".$workflow_cc_kian[$i][0] && $kian_cc_stat == 2){	//参照ボタン表示
    ...
    //              $workflow_html .= "<td align=center rowspan=1>".f_staff_name_get($workflow_data[$i][0][0])."</td>\n";
    //承認人数分作成
                    $workflow_html .= "<td align=center rowspan=1 cellspacing=0 cellpadding=0>\n";
                }else{
                    $workflow_html .= "<td align=center rowspan=".$workflow_cc_data[$i].">".$i."</td>\n";
    ...
    //承認人数分作成
		...
		$comment_f = 0;
                // rowspan=0の暫定対応 2018/04/18 oota
                if($workflow_cc_data[$i] == 0){
                    $workflow_html .= "<td align=center rowspan=1>\n";
                }else{
		...
		$workflow_html .= "</td>\n";
                ...
                if($workflow_cc_data[$i] == 0){
                    $workflow_html .= "<td align=center rowspan=1>".$workflow_data[$i][0][5][0]." / ".$workflow_data[$i][0][4][0]."</td>\n";
                    $workflow_html .= "<td align=center rowspan=1 align=left>\n";
                }else{
                    $workflow_html .= "<td align=center rowspan=".$workflow_cc_data[$i].">".$workflow_data[$i][0][5][0]." / ".$workflow_data[$i][0][4][0]."</td>\n";
...
//		$workflow2_html .= "<a href=\"#\" onclick=\"stop()\"><span class=\"undo\" /></span>取り戻す</a>\n";
	...
	if (($status == 2 || $status == 3 || $status == 4) && $input_user == $staff_code_tmp){	//再利用ボタンを表示
		...
		if ($remand_f == 0){
			$workflow2_html .= "<br /><p align=center><input type=\"button\" onclick=\"location.href='apply.php?workflow_id=".$id."&remand=1&remand_no=1'\" value=\"再申請\" class=\"confirm\" />\n";
			$f = 1;
		}
	}
/*
			...
			while($row = $rows[$cnt]) {
				...
				switch($data1)
				{
					...
					case 3:	//td
						if ($data3 != "none"){
							if ($data11 != "none"){
								$html .= "<td align='".$data3."' width='".$data6."' height='".$data7."' rowspan='".$data4."' colspan='".$data5."' style='".$data11."'>\n";
							}else{
								$html .= "<td align='".$data3."' width='".$data6."' height='".$data7."' rowspan='".$data4."' colspan='".$data5."'>\n";
							}
						}else{
							if ($data11 != "none"){
								$html .= "<td width='".$data6."' height='".$data7."' rowspan='".$data4."' colspan='".$data5."' style='".$data11."'>\n";
							}else{
								...
								if ($data16 != "none"){
									if ($data11 == "number"){
										$html .= "<input type='text' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."' style='text-align: right;'>&nbsp;".$data16."\n";
                                                                                ...
                                                                                if($_SESSION["SCHEMA"] == "akinai"){
                                                                                    if($data10 == "kingaku2101" || $data10 == "kingaku2102" || $data10 == "kingaku2103" || $data10 == "kingaku2104" || $data10 == "kingaku2105"
                                                                                            || $data10 == "kingaku2106" || $data10 == "kingaku2107" || $data10 == "kingaku2108" || $data10 == "kingaku2109" || $data10 == "kingaku2110"
                                                                                            || $data10 == "kingaku2111" || $data10 == "kingaku2112" || $data10 == "kingaku2113" || $data10 == "kingaku2114" || $data10 == "kingaku2115"
                                                                                            || $data10 == "kingaku2116" || $data10 == "kingaku2117" || $data10 == "kingaku2118" || $data10 == "kingaku2119" || $data10 == "kingaku2120" || $data10 == "kingaku2121"
                                                                                            ){
                                                                                        // 社員交通費精算
                                                                                        $html .= "<input type='text' onblur='goukeiCal1(this)' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."'>&nbsp;".$data16."\n";
                                                                                    }else if ( $data10 == "18_h_price_1" || $data10 == "18_h_price_2" || $data10 == "18_h_price_3" || $data10 == "18_h_price_4" || $data10 == "18_h_price_5"
                                                                                            || $data10 == "18_h_price_6" || $data10 == "18_h_price_7" || $data10 == "18_h_price_8" || $data10 == "18_h_price_9" || $data10 == "18_h_price_10"
                                                                                            || $data10 == "18_h_price_11" || $data10 == "18_h_price_12" || $data10 == "18_h_price_13" || $data10 == "18_h_price_14" || $data10 == "18_h_price_15"
                                                                                            || $data10 == "18_h_price_16" || $data10 == "18_h_price_17" || $data10 == "18_h_price_18" || $data10 == "18_h_price_19" || $data10 == "18_h_price_20"
                                                                                            || $data10 == "18_h_price_21" || $data10 == "18_h_price_22" || $data10 == "18_h_price_23"
                                                                                            ){
                                                                                        // アルバイトヘルプ交通費・ヘルプ手当
                                                                                        $html .= "<input type='text' onblur='goukeiCal2(this)' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."'>&nbsp;".$data16."\n";
                                                                                    }else if ( $data10 == "nisuu2501" || $data10 == "nisuu2502" || $data10 == "nisuu2503" || $data10 == "nisuu2504" || $data10 == "nisuu2505"
                                                                                            || $data10 == "tanka2501" || $data10 == "tanka2502" || $data10 == "tanka2503" || $data10 == "tanka2504" || $data10 == "tanka2505"
                                                                                            || $data10 == "shoukei2501" || $data10 == "shoukei2502" || $data10 == "shoukei2503" || $data10 == "shoukei2504" || $data10 == "shoukei2505"
                                                                                            ){
                                                                                        // 出張手当
                                                                                        $html .= "<input type='text' onblur='goukeiCal3(this)' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."'>&nbsp;".$data16."\n";
                                                                                    }else if($data10 == "goukei2101" || $data10 == "18_h_goukei" || $data10 == "gounisuu2501" || $data10 == "goutanka2501" || $data10 == "goushoukei2501"){
                                                                                        // 合計パーツ
                                                                                        $html .= "<input type='text' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."' readonly >&nbsp;".$data16."\n";
                                                                                    }else{
                                                                                        // 通常パーツ
									...
									if ($data11 == "number"){
										$html .= "<input type='text' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."' style='text-align: right;'>\n";
                                                                                ...
                                                                                // アキナイ向け合計計算用スクリプト対応(一時的)
                                                                                if($_SESSION["SCHEMA"] == "akinai"){
                                                                                    if ( $data10 == "nisuu2501" || $data10 == "nisuu2502" || $data10 == "nisuu2503" || $data10 == "nisuu2504" || $data10 == "nisuu2505"
                                                                                            || $data10 == "tanka2501" || $data10 == "tanka2502" || $data10 == "tanka2503" || $data10 == "tanka2504" || $data10 == "tanka2505"
                                                                                            ){
                                                                                        // 出張手当
                                                                                        $html .= "<input type='text' onblur='goukeiCal3(this)' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."'>\n";
                                                                                    }else if($data10 == "shoukei2501" || $data10 == "shoukei2502" || $data10 == "shoukei2503" || $data10 == "shoukei2504" || $data10 == "shoukei2505" 
                                                                                            || $data10 == "gounisuu2501" || $data10 == "goutanka2501" || $data10 == "goushoukei2501"){
                                                                                        // 合計パーツ
                                                                                        $html .= "<input type='text' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."' readonly>\n";
                                                                                    }else{
                                                                                        // 通常パーツ
...
/*
							if ($data3 != "none"){
								if ($data11 != "none"){
									$html .= "<td align='".$data3."' width='".$data6."' height='".$data7."' rowspan='".$data4."' colspan='".$data5."' style='word-break:break-all;".$data11."'>\n";
								}else{
									$html .= "<td align='".$data3."' width='".$data6."' height='".$data7."' rowspan='".$data4."' colspan='".$data5."' style='word-break:break-all;'>\n";
								}
							}else{
								if ($data11 != "none"){
									$html .= "<td width='".$data6."' height='".$data7."' rowspan='".$data4."' colspan='".$data5."' style='word-break:break-all;".$data11."'>\n";
								}else{
									$html .= "<td width='".$data6."' height='".$data7."' rowspan='".$data4."' colspan='".$data5."' style='word-break:break-all;'>\n";
								}
							}
*/
						...
						case 10:	//input
							switch($data12)
							{
								...
								case 3:	//type = checkbox
									if ($post_name_save != $data10){
										if (f_post_name_data_get($workflow_id, $data10) == ""){
											$html .= "□".$data16;
										}else{
											$html .= "■".f_post_name_data_get($workflow_id, $data10);
										}
						...
						case 30:	//date
							if ($data16 != "none"){
								$html .= f_post_name_data_get($workflow_id, $data10)."&nbsp;".$data16;
							}else{
								$html .= f_post_name_data_get($workflow_id, $data10);
							}
							break;
...
function workflow_format_write_mobile($wdoc_no, $workflow_id)
{
					...
					case 10:	//input
						switch($data12)
						{
							case 2:	//type = text
								...
								if ($data16 != "none"){
									if ($data11 == "number"){
                                                                        	$html .= "<input type='text' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."'>&nbsp;".$data16."\n";
									}else{
                                                                                // アキナイ向け合計計算用スクリプト対応(一時的)
                                                                                if($_SESSION["SCHEMA"] == "akinai"){
                                                                                    if($data10 == "kingaku2101" || $data10 == "kingaku2102" || $data10 == "kingaku2103" || $data10 == "kingaku2104" || $data10 == "kingaku2105"
                                                                                        ...
                                                                                        // 社員交通費精算
                                                                                        $html .= "<input type='text' onblur='goukeiCal1(this)' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."'>&nbsp;".$data16."\n";
                                                                                    }else if ( $data10 == "18_h_price_1" || $data10 == "18_h_price_2" || $data10 == "18_h_price_3" || $data10 == "18_h_price_4" || $data10 == "18_h_price_5"
                                                                                            || $data10 == "18_h_price_6" || $data10 == "18_h_price_7" || $data10 == "18_h_price_8" || $data10 == "18_h_price_9" || $data10 == "18_h_price_10"
                                                                                            || $data10 == "18_h_price_11" || $data10 == "18_h_price_12" || $data10 == "18_h_price_13" || $data10 == "18_h_price_14" || $data10 == "18_h_price_15"
                                                                                            || $data10 == "18_h_price_16" || $data10 == "18_h_price_17" || $data10 == "18_h_price_18" || $data10 == "18_h_price_19" || $data10 == "18_h_price_20"
                                                                                        ...
                                                                                        // アルバイトヘルプ交通費・ヘルプ手当
                                                                                        $html .= "<input type='text' onblur='goukeiCal2(this)' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."'>&nbsp;".$data16."\n";
                                                                                    }else if ( $data10 == "nisuu2501" || $data10 == "nisuu2502" || $data10 == "nisuu2503" || $data10 == "nisuu2504" || $data10 == "nisuu2505"
                                                                                            || $data10 == "tanka2501" || $data10 == "tanka2502" || $data10 == "tanka2503" || $data10 == "tanka2504" || $data10 == "tanka2505"
                                                                                            || $data10 == "shoukei2501" || $data10 == "shoukei2502" || $data10 == "shoukei2503" || $data10 == "shoukei2504" || $data10 == "shoukei2505"
                                                                                            ){
                                                                                        // 出張手当
                                                                                        $html .= "<input type='text' onblur='goukeiCal3(this)' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."'>&nbsp;".$data16."\n";
                                                                                    }else if($data10 == "goukei2101" || $data10 == "18_h_goukei" || $data10 == "gounisuu2501" || $data10 == "goutanka2501" || $data10 == "goushoukei2501"){
                                                                                        // 合計パーツ
                                                                                        $html .= "<input type='text' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."' readonly >&nbsp;".$data16."\n";
                                                                                    }else{
                                                                                        // 通常パーツ
                                                                                        $html .= "<input type='text' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."'>&nbsp;".$data16."\n";
                                                                                    }
									...
									if ($data11 == "number"){
                                                                                $html .= "<input type='text' id='".$data8."' name='".$data10."' style='width:80%;text-align: right;' value='".f_post_name_data_get($workflow_id, $data10)."'>\n";
									}else{
                                                                                // アキナイ向け合計計算用スクリプト対応(一時的)
                                                                                if($_SESSION["SCHEMA"] == "akinai"){
                                                                                    if ( $data10 == "nisuu2501" || $data10 == "nisuu2502" || $data10 == "nisuu2503" || $data10 == "nisuu2504" || $data10 == "nisuu2505"
                                                                                        ...
                                                                                        // 出張手当
                                                                                        $html .= "<input type='text' onblur='goukeiCal3(this)' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."'>\n";
                                                                                    }else if($data10 == "shoukei2501" || $data10 == "shoukei2502" || $data10 == "shoukei2503" || $data10 == "shoukei2504" || $data10 == "shoukei2505" 
                                                                                            || $data10 == "gounisuu2501" || $data10 == "goutanka2501" || $data10 == "goushoukei2501"){
                                                                                        // 合計パーツ
                                                                                        $html .= "<input type='text' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."' readonly>\n";
                                                                                    }else{
                                                                                        // 通常パーツ
                                                                                        $html .= "<input type='text' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."'>\n";
                                                                                    }
                                                                                }else{
                                                                                    // アキナイ以外
                                                                                    $html .= "<input type='text' id='".$data8."' name='".$data10."' style='width:80%;' value='".f_post_name_data_get($workflow_id, $data10)."'>\n";
						...
						break;
					case 20:	//textarea
						$html .= "<textarea id='".$data8."' name='".$data10."' rows='".$data4."' style='width:100%;'>".nl2br(f_post_name_data_get($workflow_id, $data10))."</textarea>\n";
						$flag = 1;
						break;
					case 30:	//date
						$html .= "<input type='text' id='".$data8."' name='".$data10."' size='".$data14."' value='".f_post_name_data_get($workflow_id, $data10)."'>\n";
						...
						case 10:	//input
							switch($data12)
							{
								case 2:	//type = text
									...
									$content .= htmlspecialchars(f_post_name_data_get($workflow_id, $data10));
									break;
								case 4:	//type = dsp 表示のみ
							...
							break;
						case 20:	//textarea
							$tmp = nl2br(htmlspecialchars(f_post_name_data_get($workflow_id, $data10)));
							if($tmp != ""){
								$content .= $tmp."&nbsp;";
								$flag = 1;
							}
