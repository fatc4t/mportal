<?php
    /**
     * @file      商品JICFS分類コントローラ
     * @author    川橋
     * @date      2019/01/22
     * @version   1.00
     * @note      商品JICFS分類の新規登録/修正/削除を行う
     */

    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // 顧客モデル
    require './Model/Mst5401.php';
    /**
     * 商品JICFS分類コントローラクラス
     * @note   ログイン時のリクエストを振り分ける
     */
    class Mst5401Controller extends BaseController
    {
        /**
         * コンストラクタ
         */
...
//                $mst5401->deldata($_GET["orgnid"], $_GET["sectcd"]);            
//                $_GET["orgnid"]="";
//                $_GET["sectcd"]="";
//                $this->initialDisplay();
//                $Log->trace("END   delAction");
...
//
//            // 店舗コードプルダウン
//            $abbreviatedNameList        = $mst5401->setPulldown->getSearchOrganizationList( 'reference', true, true );
//            
//            // 商品部門一覧データ取得
//            $mst5401List        = $mst5401->getMst5401List();
//            $mst5401_searchdata = $mst5401->searchMst5401Data();
//            for ($intL = 0; $intL < Count($mst5401_searchdata); $intL ++) {
//                $mst5401_searchdata[$intL]['abbreviated_name'] = '';
//                foreach ($abbreviatedNameList as $abbreviatedName) {
//                    if ($mst5401_searchdata[$intL]['organization_id'] === $abbreviatedName['organization_id']) {
...
//                        $abbreviated_name = ltrim($abbreviated_name, '└');
//                        $abbreviated_name = ltrim($abbreviated_name, '　');
//                        $mst5401_searchdata[$intL]['abbreviated_name'] = $abbreviated_name;
//                        break;
//                    }
//                }
//            }
//
//            require_once './View/Mst5401Search.html';
...
//            // 一覧表用検索項目
//            if ($ssect_cd != "" ){
//                    $mst5401_getdata = $mst5401->getMst5401Data($ssect_cd);
//            }
//            if ($iorgn_id != "" && $ssect_cd != "" ){
//                    $mst5401_getdata = $mst5401->getMst5401Data($iorgn_id, $ssect_cd);
//            }
//            else if ($iorgn_id != ""){
//                // 登録画面で組織プルダウン変更時
//                $mst5401_getdata = array(
...
//
//            // 部門分類一覧データ取得
//            $mst5401_alldata = $mst5401->getMst5401Data($iorgn_id);
//            
//            // 商品一覧データ取得
//            $mst0201_alldata = $mst5401->getMst0201Data($iorgn_id);
//            
//            // システムマスタ取得
//            $mst0010_getdata = $mst5401->getMst0010Data($iorgn_id);
//            // 部門コード入力長
...
//
//            require_once './View/Mst5401Panel.html';
//
...
//            $_SESSION["PAGE_NO"] = 1;
        ...
        public function templateAction($aryErrMsg, $organization_id = '')
        {
            ...
            // 商品JICFS分類一覧データ取得
            //$data           = $mst5401->get_mst5401_data();
            $data = array();
            if ($orgn_id !== ''){
                $data           = $mst5401->get_mst5401_data($orgn_id);
            }

            $title          = array("コード","JICFS分類名","JICFS分類名カナ");
            $key            = array("jicfs_class_cd","jicfs_class_nm","jicfs_class_kn");
            $pr_key_id      = array("jicfs_class_cd");
            $pr_key_list    = "jicfs_class_cd";
            $addrow         = 1;
            $save           = 1;
            $controller     = 'Product';
            $pagetitle      = '商品管理　JICFS分類マスタ';
            $destination    = 'Mst5401';
...
//                $mst5401->Delete($organization_id, $del_data, $aryErrMsg);
//            }
//            // 新規
//            if (isset($_POST['new_data']) === true){
//                $new_data = json_decode($_POST['new_data'], true);
//                //$mst5401->Insert($new_data);
//                $mst5401->Insert($organization_id, $new_data, $aryErrMsg);
//            }
//            // 更新
//            if (isset($_POST['upd_data']) === true){
//                $upd_data = json_decode($_POST['upd_data'], true);
//                //$mst5401->Update($upd_data);
//                $mst5401->Update($organization_id, $upd_data, $aryErrMsg);
//            }
            // 登録・更新・削除一括処理
            $mst5401->BatchRegist($_POST, $aryErrMsg);
