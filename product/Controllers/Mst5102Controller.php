<?php
     ...
     * @date      2019/01/22
     * @version   1.00
     * @note      商品入数構成の新規登録/修正/削除を行う
     */

    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // 顧客モデル
    require './Model/Mst5102.php';
    /**
     * 商品入数構成コントローラクラス
     * @note   ログイン時のリクエストを振り分ける
     */
    class Mst5102Controller extends BaseController
    {
        /**
         * コンストラクタ
         */
...
//                $mst5102->deldata($_GET["orgnid"], $_GET["sectcd"]);            
//                $_GET["orgnid"]="";
//                $_GET["sectcd"]="";
//                $this->initialDisplay();
//                $Log->trace("END   delAction");
...
//
//            // 店舗コードプルダウン
//            $abbreviatedNameList        = $mst5102->setPulldown->getSearchOrganizationList( 'reference', true, true );
//            
//            // 商品部門一覧データ取得
//            $mst5102List        = $mst5102->getMst5102List();
//            $mst5102_searchdata = $mst5102->searchMst5102Data();
//            for ($intL = 0; $intL < Count($mst5102_searchdata); $intL ++) {
//                $mst5102_searchdata[$intL]['abbreviated_name'] = '';
//                foreach ($abbreviatedNameList as $abbreviatedName) {
//                    if ($mst5102_searchdata[$intL]['organization_id'] === $abbreviatedName['organization_id']) {
...
//                        $abbreviated_name = ltrim($abbreviated_name, '└');
//                        $abbreviated_name = ltrim($abbreviated_name, '　');
//                        $mst5102_searchdata[$intL]['abbreviated_name'] = $abbreviated_name;
//                        break;
//                    }
//                }
//            }
...
//            // 一覧表用検索項目
//            if ($ssect_cd != "" ){
//                    $mst5102_getdata = $mst5102->getMst5102Data($ssect_cd);
//            }
//            if ($iorgn_id != "" && $ssect_cd != "" ){
//                    $mst5102_getdata = $mst5102->getMst5102Data($iorgn_id, $ssect_cd);
//            }
//            else if ($iorgn_id != ""){
//                // 登録画面で組織プルダウン変更時
//                $mst5102_getdata = array(
...
//
//            // 分類一覧データ取得
//            $mst5102_alldata = $mst5102->getMst5102Data($iorgn_id);
//            
//            // 商品一覧データ取得
//            $mst0201_alldata = $mst5102->getMst0201Data($iorgn_id);
//            
//            // システムマスタ取得
//            $mst0010_getdata = $mst5102->getMst0010Data($iorgn_id);
//            // 部門コード入力長
...
//
//            require_once './View/Mst5102Panel.html';
//
...
//            $_SESSION["PAGE_NO"] = 1;
        ...
        public function templateAction($aryErrMsg, $aryKeys = array())
        {
            ...
            // 分類種別プルダウン変更時
            if ($_GET['orgnid']){
                // from Mst5102template1.html
                $aryKeys = array();
                // 組織ID
                $aryKeys['organization_id'] = $_GET['orgnid'];
                // 構成商品コード
                $aryKeys['setprod_cd'] = $_GET['setprod'];
            }
            if ($_GET["search"]) {
                // from Mst5102Search.html
                $param = explode(":", $_GET['search']);
                $aryKeys = array();
                $aryKeys['organization_id'] = $param[0];
                $aryKeys['setprod_cd']      = $param[1];                
            }

            // 商品入数構成一覧データ取得
            $mst0201_data = array();
            $setprod_nm = "";
            //$data           = $mst5102->get_mst5102_data();
            $data = array();
            if (count($aryKeys) > 0){
                // 商品マスタ取得
                $mst0201_data = $mst5102->get_mst0201_data( $aryKeys );
                if (count($mst0201_data) === 0){
                    $aryErrMsg[] = "構成商品コードがマスタにありません。";
                }
                else{
                    // セット商品コード存在チェック
                    if ($mst5102->SetProdIsExist($aryKeys['organization_id'], $aryKeys['setprod_cd'], "") === true){
                        $aryErrMsg[] = "他の商品入数で親コードとして登録されています。";
                        $mst0201_data = array();    // 初期化
                    }
                    else{
                        $setprod_nm     = $mst0201_data[0]["prod_nm"];
                        $data           = $mst5102->get_mst5102_data($aryKeys);
                    }
                }
            }

            $title          = array("","セット商品コード","商品名カナ略","容量","商品売価","商品原価","入数");
            $key            = array("order_object","prod_cd","prod_kn_rk","prod_capa_nm","saleprice","costprice","setprod_amount");
            $pr_key_id      = array("prod_cd");
            $pr_key_list    = "prod_cd";
            $addrow         = 1;
            $save           = 1;
            $controller     = 'Product';
            $pagetitle      = '商品管理　商品入数構成マスタ';
            $destination    = 'Mst5102';

            // 商品一覧データ取得
            $mst0201List        = $mst5102->getMst0201List();
            $mst0201_searchdata = $mst5102->searchMst0201Data();

            // 店舗コードプルダウン
            $abbreviatedNameList = $mst5102->setPulldown->getSearchOrganizationList( 'reference', true, true );
            
            //require_once '../pagetemplate1/View/template1.html';
            require_once '../product/View/Mst5102template1.html';
...
//            }
        ...
        public function alldeleteAction()
        {
            ...
            // 削除
            if ($mst5102->AllDelete($aryKeys, $aryErrMsg) === true){
                // 初期化
                unset($_GET);
                $aryKeys = array();
            }
