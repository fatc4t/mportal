<?php
     ...
     * @date      2019/01/22
     * @version   1.00
     * @note      商品自社分類の新規登録/修正/削除を行う
     */

    // BaseController.phpを読み込む
    require './Controllers/Common/BaseController.php';
    // 顧客モデル
    require './Model/Mst5501.php';
    /**
     * 商品自社分類コントローラクラス
     * @note   ログイン時のリクエストを振り分ける
     */
    class Mst5501Controller extends BaseController
    {
        /**
         * コンストラクタ
         */
...
//                $mst5501->deldata($_GET["orgnid"], $_GET["sectcd"]);            
//                $_GET["orgnid"]="";
//                $_GET["sectcd"]="";
//                $this->initialDisplay();
//                $Log->trace("END   delAction");
...
//
//            // 店舗コードプルダウン
//            $abbreviatedNameList        = $mst5501->setPulldown->getSearchOrganizationList( 'reference', true, true );
//            
//            // 商品部門一覧データ取得
//            $mst5501List        = $mst5501->getMst5501List();
//            $mst5501_searchdata = $mst5501->searchMst5501Data();
//            for ($intL = 0; $intL < Count($mst5501_searchdata); $intL ++) {
//                $mst5501_searchdata[$intL]['abbreviated_name'] = '';
//                foreach ($abbreviatedNameList as $abbreviatedName) {
//                    if ($mst5501_searchdata[$intL]['organization_id'] === $abbreviatedName['organization_id']) {
...
//                        $abbreviated_name = ltrim($abbreviated_name, '└');
//                        $abbreviated_name = ltrim($abbreviated_name, '　');
//                        $mst5501_searchdata[$intL]['abbreviated_name'] = $abbreviated_name;
//                        break;
//                    }
//                }
//            }
//
//            require_once './View/Mst5501Search.html';
...
//            // 一覧表用検索項目
//            if ($ssect_cd != "" ){
//                    $mst5501_getdata = $mst5501->getMst5501Data($ssect_cd);
//            }
//            if ($iorgn_id != "" && $ssect_cd != "" ){
//                    $mst5501_getdata = $mst5501->getMst5501Data($iorgn_id, $ssect_cd);
//            }
//            else if ($iorgn_id != ""){
//                // 登録画面で組織プルダウン変更時
//                $mst5501_getdata = array(
...
//
//            // 部門分類一覧データ取得
//            $mst5501_alldata = $mst5501->getMst5501Data($iorgn_id);
//            
//            // 商品一覧データ取得
//            $mst0201_alldata = $mst5501->getMst0201Data($iorgn_id);
//            
//            // システムマスタ取得
//            $mst0010_getdata = $mst5501->getMst0010Data($iorgn_id);
//            // 部門コード入力長
...
//
//            require_once './View/Mst5501Panel.html';
//
...
//            $_SESSION["PAGE_NO"] = 1;
        ...
        public function templateAction($aryErrMsg, $organization_id = '')
        {
            ...
            // 商品自社分類一覧データ取得
            //$data           = $mst5501->get_mst5501_data();
            $data = array();
            if ($orgn_id !== ''){
                $data           = $mst5501->get_mst5501_data($orgn_id);
            }

            $title          = array("コード","自社分類名","自社分類名カナ");
            $key            = array("priv_class_cd","priv_class_nm","priv_class_kn");
            $pr_key_id      = array("priv_class_cd");
            $pr_key_list    = "priv_class_cd";
            $addrow         = 1;
            $save           = 1;
            $controller     = 'Product';
            $pagetitle      = '商品管理　自社分類マスタ(クラス)';
            $destination    = 'Mst5501';
...
//                $mst5501->Delete($organization_id, $del_data, $aryErrMsg);
//            }
//            // 新規
//            if (isset($_POST['new_data']) === true){
//                $new_data = json_decode($_POST['new_data'], true);
//                //$mst5501->Insert($new_data);
//                $mst5501->Insert($organization_id, $new_data, $aryErrMsg);
//            }
//            // 更新
//            if (isset($_POST['upd_data']) === true){
//                $upd_data = json_decode($_POST['upd_data'], true);
//                //$mst5501->Update($upd_data);
//                $mst5501->Update($organization_id, $upd_data, $aryErrMsg);
//            }
            // 登録・更新・削除一括処理
            $mst5501->BatchRegist($_POST, $aryErrMsg);
