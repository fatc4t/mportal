<?php
     ...
     */
    class Mst5401 extends BaseProduct
    {
        /**
         * コンストラクタ
         */
...
//         * @return   SQLの実行結果
//         */
//        public function getMst5401Data( $type_cd )
//        public function getMst5401Data( $organization_id, $type_cd )
//        {
//            global $DBA, $Log; // グローバル変数宣言
//            $Log->trace("START getMst5401Data");
//
//            $sql  = " SELECT ";
//            $sql .= "     INSDATETIME";
//            $sql .= "    ,UPDDATETIME";
...
//
//            // 一覧表を格納する空の配列宣言
//            $mst5401DataList = array();
//
//            // データ取得ができなかった場合、空の配列を返す
//            if( $result === false )
//            {
//                $Log->trace("END getMst5401Data");
//                return $mst5401DataList;
//            }
//
//            // 取得したデータ群を配列に格納
//            while ( $data = $result->fetch(PDO::FETCH_ASSOC) )
//            {
//                $mst5401DataList = $data;
//            }
//
//            $Log->trace("END getMst5401Data");
//
...
//        }
         ...
         * @return   SQLの実行結果
         */
        public function searchMst5401Data()
        {
            global $DBA, $Log; // searchMst5401Data変数宣言
            $Log->trace("START searchMst5401Data");

//            $sql = ' SELECT jicfs_class_cd,jicfs_class_nm,jicfs_class_kn from mst5401 where 1 = :jicfs_class_cd order by jicfs_class_cd desc';
            $sql = ' SELECT organization_id, jicfs_class_cd,jicfs_class_nm,jicfs_class_kn from mst5401 where 1 = :jicfs_class_cd order by jicfs_class_cd desc';
            ...
            if( $result === false )
            {
                $Log->trace("END searchMst5401Data");
                return $mst5401DataList;
            }
            ...
            while ( $data = $result->fetch(PDO::FETCH_ASSOC) )
            {
                array_push( $mst5401DataList, $data);
            }
         ...
         * @return   SQLの実行結果
         */
            public function getMst5401List()
        {
            global $DBA, $Log; // グローバル変数宣言
            $Log->trace("START getMst5401List");

//            $sql = " SELECT string_agg(jicfs_class_cd,',') as list from mst5401 where 1 = :jicfs_class_cd ";            
            $sql = " SELECT string_agg(organization_id||':'||jicfs_class_cd,',') as list from mst5401 where 1 = :jicfs_class_cd ";            
            $searchArray = array(':jicfs_class_cd' => 1);
         ...
         * @return   SQLの実行結果
         */
        //public function get_mst5401_data()
        public function get_mst5401_data( $organization_id )
        {
            global $DBA, $Log; // グローバル変数宣言
            $Log->trace("START get_mst5401_data");

            $sql = ' select jicfs_class_cd,jicfs_class_nm,jicfs_class_kn from mst5401';
            //$sql .= '  WHERE 1 = :jicfs_class_cd  ';
            $sql .= '  WHERE organization_id = :organization_id  ';
            ...
            if( $result === false )
            {
                $Log->trace("END get_mst5401_data");
                return $mst5401DataList;
            }
        ...
        public function Insert($organization_id, $data, &$aryErrMsg)
        {
            ...
            $query .= "    ,JICFS_CLASS_CD    ";
            $query .= "    ,JICFS_CLASS_NM    ";
            $query .= "    ,JICFS_CLASS_KN    ";
            $query .= "  ) values (           ";
            $query .= "     :INSUSER_CD       ";
            $query .= "    ,:INSDATETIME      ";
            $query .= "    ,:UPDUSER_CD       ";
            ...
            for ($intL = 0; $intL < count($data); $intL ++){
                $sind = '"'.$intL.'"';
                if (is_numeric($data[$sind]['jicfs_class_cd']) === false){
                    continue;
                }
                $param = array();
                ...
                $param[":JICFS_CLASS_CD"]   = $data[$sind]["jicfs_class_cd"];
                $param[":JICFS_CLASS_NM"]   = $data[$sind]["jicfs_class_nm"];
                $param[":JICFS_CLASS_KN"]   = $data[$sind]["jicfs_class_kn"];

                $result = $DBA->executeSQL($query, $param);
                if ($result === false){
                    $aryErrMsg[] = '追加処理中にエラーが発生しました。(JICFS分類コード：'.$data[$sind]["jicfs_class_cd"].')';
        ...
        public function Update($organization_id, $data, &$aryErrMsg)
        {
            ...
            for ($intL = 0; $intL < count($data); $intL ++){
                ...
                $updkey_cd = '';
                $sind = '"'.$intL.'"';
                if (is_numeric($data[$sind]['updkey_jicfs_class_cd'])){
                    foreach ($data[$sind] as $key => $value){
                        $do_query = 0;
                        if ($key === 'updkey_jicfs_class_cd'){
                            $updkey_cd = $value;
                            $param[":updkey_jicfs_class_cd"] = $updkey_cd;
                            continue;
                        }
                        if (is_null($value) === false){
                            if ($key === 'jicfs_class_cd'){
                                if (is_numeric($value) === false){
                                    continue;
                                }
                                // 他テーブル使用状況チェック
                                if ($this->UsedTableCheck($organization_id, $updkey_cd) === false){
                                    $aryErrMsg[] = '他のマスタで使用されているためコード変更できません。(JICFS分類コード：'.$updkey_cd.')';
                                    continue;
                                }
                            }
                            $do_query = 1;
                    ...
                    $query .= " and   jicfs_class_cd    = :updkey_jicfs_class_cd";

                    $result = $DBA->executeSQL($query, $param);
                    if ($result === false){
                        $aryErrMsg[] = '更新処理中にエラーが発生しました。(JICFS分類コード：'.$updkey_cd.')';
        ...
        public function Delete($organization_id, $data, &$aryErrMsg){
            ...
            $query  = "";
            $query .= " delete from mst5401 ";
            $query .= " where organization_id = :organization_id";
            $query .= " and   jicfs_class_cd = :jicfs_class_cd";
            ...
            for ($intL = 0; $intL < count($data); $intL ++){
                ...
                if ($this->UsedTableCheck($organization_id, $data[$sind]['priv_class_cd']) === false){
                    $aryErrMsg[] = '他のマスタで使用されているため削除できません。(JICFS分類コード：'.$data[$sind]["jicfs_class_cd"].')';
                    continue;
                }
                $param = array();
                $param[":organization_id"]  = $organization_id;
                ...
                $result = $DBA->executeSQL($query, $param);
                if ($result === false){
                    $aryErrMsg[] = '削除処理中にエラーが発生しました。(JICFS分類コード：'.$data[$sind]["jicfs_class_cd"].')';
         ...
         * @return boolean
         */
        function UsedTableCheck($organization_id, $jicfs_class_cd) {
            ...
            $sqlArray = array(
                // 商品部門マスタ
                'SELECT COUNT(prod_cd) AS CNT FROM mst0201 WHERE organization_id = :organization_id AND jicfs_class_cd = :jicfs_class_cd',  // 商品マスタ
                'SELECT COUNT(prod_cd) AS CNT FROM mst0211 WHERE organization_id = :organization_id AND jicfs_class_cd = :jicfs_class_cd',  // 予約商品マスタ
                'SELECT COUNT(hideseq) AS CNT FROM trn1702 WHERE organization_id = :organization_id AND jicfs_class_cd = :jicfs_class_cd',  // 棚卸確定データ明細
                'SELECT COUNT(hideseq) AS CNT FROM jsk4120 WHERE organization_id = :organization_id AND jicfs_class_cd = :jicfs_class_cd',  // 顧客購買履歴
            );
...
//         * @return   SQLの実行結果
//         */
//        public function getMst5401Data( $organization_id )
//        {
//            global $DBA, $Log; // グローバル変数宣言
//            $Log->trace("START getMst5401Data");
//
...
//            }
//
//            $sql = ' select organization_id, type_cd, type_nm  from mst5401';
//            $sql .= "  WHERE disabled = :disabled  ";
//            $sql .= "  AND   organization_id = :organization_id  ";
