...
function setDataForAjax( data, path, replacementArea, async )
{
    ...
    // リプレースエリアが、全画面更新時の時 
    if( (replacementArea == 'ajaxScreenUpdate' && -1 != path.indexOf("searchProd"))   ||
        (replacementArea == 'ajaxScreenUpdate' && -1 != path.indexOf("addInput"))   ||
        (replacementArea == 'ajaxScreenUpdate' && -1 != path.indexOf("showSales"))  ||
        (replacementArea == 'ajaxScreenUpdate' && -1 != path.indexOf("changeinput"))|| 
        (replacementArea == 'ajaxScreenUpdate' && -1 != path.indexOf("addRow"))       )
    {
        // submit用のFROMを作成
    ...
    $.ajax({
        ...
        success: function( data, dataType )
        {
            ...
            else if( -1 == data.indexOf(replacementArea) )
            {
                ...
                {
                    if (-1 != path.indexOf("searchCustomerClassification") || -1 != path.indexOf("searchCodeName") || -1 !== path.indexOf("searchProd")) 
                    {
            ...
            else
            {
                ...
                if( -1 == path.indexOf("search") && -1 == path.indexOf("inputArea") && -1 == path.indexOf("checkLaborStandardsActInfo") )
                {
                    ...
                    if( 0 < path.indexOf("addInput") ||  0 < path.indexOf("addRow"))
                    {
                        document.write(data);
                        document.close();
                        return;
